...
_RUST_NUMERIC_SCALAR_TYPES = (int, float, np.integer, np.floating)
...
def _can_use_rust_merge(gate_excluded, marked):
    if not isinstance(gate_excluded, np.ndarray) or not isinstance(marked, np.ndarray):
        return False
    return (
        gate_excluded.dtype == np.bool_
        and marked.dtype == np.bool_
        and gate_excluded.ndim == 2
        and marked.ndim == 2
        and gate_excluded.shape == marked.shape
        and gate_excluded.flags.c_contiguous
        and marked.flags.c_contiguous
    )


def _coerce_rust_numeric_scalar(value):
    if isinstance(value, (bool, np.bool_)):
        return None
    if not isinstance(value, _RUST_NUMERIC_SCALAR_TYPES):
        return None
    try:
        return float(value)
...
def _coerce_rust_transition_scalar(value):
...
def _can_use_rust_compare_merge(gate_excluded, data, data_mask, value, op, exclude_masked):
    if op not in ("or", "and", "new") or exclude_masked not in [True, False]:
        return None
    value = _coerce_rust_numeric_scalar(value)
    if value is None:
        return None
    if not (
        type(gate_excluded) is np.ndarray
        and type(data) is np.ndarray
        and type(data_mask) is np.ndarray
        and gate_excluded.ndim == 2
        and data.ndim == 2
        and data_mask.ndim == 2
        and gate_excluded.shape == data.shape
        and gate_excluded.shape == data_mask.shape
        and gate_excluded.dtype == np.bool_
        and data.dtype == np.float64
        and data_mask.dtype == np.bool_
        and gate_excluded.flags.c_contiguous
        and data.flags.c_contiguous
        and data_mask.flags.c_contiguous
    ):
        return None
    return value


def _can_use_rust_interval_merge(gate_excluded, data, data_mask, v1, v2, op, exclude_masked):
    if op not in ("or", "and", "new") or exclude_masked not in [True, False]:
        return None
    v1 = _coerce_rust_numeric_scalar(v1)
    v2 = _coerce_rust_numeric_scalar(v2)
...
def _can_use_rust_finite_merge(gate_excluded, data, data_mask, op, exclude_masked):
    if op not in ("or", "and", "new") or exclude_masked not in [True, False]:
        return False
    return (
        type(gate_excluded) is np.ndarray
...
def _can_use_rust_last_gates_merge(gate_excluded, fdata, n_gates, op, exclude_masked):
    if op not in ("or", "and", "new") or exclude_masked not in [True, False]:
        return None
    try:
        n_gates = operator.index(n_gates)
        fdata_shape = fdata.shape
    except (AttributeError, TypeError):
        return None
    intp_info = np.iinfo(np.intp)
    if n_gates < intp_info.min or n_gates > intp_info.max:
        return None
    if not (
        type(gate_excluded) is np.ndarray
        and gate_excluded.ndim == 2
        and tuple(fdata_shape) == gate_excluded.shape
        and gate_excluded.dtype == np.bool_
        and gate_excluded.flags.c_contiguous
    ):
        return None
...
def _can_use_rust_transition_merge(
    gate_excluded, transition_data, trans_value, op, exclude_masked
):
    if op not in ("or", "and", "new") or exclude_masked not in [True, False]:
        return None
    trans_value = _coerce_rust_transition_scalar(trans_value)
    if trans_value is None:
        return None
    if not (
        type(gate_excluded) is np.ndarray
        and type(transition_data) is np.ndarray
        and gate_excluded.ndim == 2
        and transition_data.ndim == 1
        and transition_data.shape[0] == gate_excluded.shape[0]
        and gate_excluded.dtype == np.bool_
        and transition_data.dtype == np.float64
        and gate_excluded.flags.c_contiguous
        and transition_data.flags.c_contiguous
        and not np.ma.isMaskedArray(transition_data)
    ):
        return None
    return trans_value
...
class GateFilter:
    ...
    def _merge(self, marked, op, exclude_masked):
        ...
        kernel = _rust_kernel("_gatefilter_merge")
        if kernel is not None and _can_use_rust_merge(self._gate_excluded, marked):
            self._gate_excluded = kernel(self._gate_excluded, marked, op_name)
            return
    ...
    def _merge_compare_threshold(
        self, fdata, value, comparator, invert_marked, op, exclude_masked
    ):
        """Compare field data against a threshold and merge in one Rust call."""
        data = np.ma.getdata(fdata)
        data_mask = np.ma.getmaskarray(fdata)
        rust_value = _can_use_rust_compare_merge(
            self._gate_excluded, data, data_mask, value, op, exclude_masked
        )
        if rust_value is None:
            return False
        kernel = _rust_kernel("_gatefilter_compare_merge")
        if kernel is None:
            return False

        self._gate_excluded = kernel(
            self._gate_excluded,
            data,
            data_mask,
            rust_value,
    ...
    def _merge_compare_interval(
        self, fdata, v1, v2, mode, inclusive, invert_marked, op, exclude_masked
    ):
        """Compare field data against an interval and merge in one Rust call."""
        data = np.ma.getdata(fdata)
        data_mask = np.ma.getmaskarray(fdata)
        rust_values = _can_use_rust_interval_merge(
            self._gate_excluded, data, data_mask, v1, v2, op, exclude_masked
        )
        if rust_values is None:
            return False
        kernel = _rust_kernel("_gatefilter_interval_merge")
        if kernel is None:
            return False

        rust_v1, rust_v2 = rust_values
        self._gate_excluded = kernel(
            self._gate_excluded,
            data,
            data_mask,
    ...
    def _merge_finite(self, fdata, op, exclude_masked):
        """Mark non-finite field data and merge in one Rust call."""
        data = np.ma.getdata(fdata)
        data_mask = np.ma.getmaskarray(fdata)
        kernel = _rust_kernel("_gatefilter_finite_merge")
        if kernel is None or not _can_use_rust_finite_merge(
            self._gate_excluded, data, data_mask, op, exclude_masked
        ):
            return False
    ...
    def _merge_last_gates(self, fdata, n_gates, op, exclude_masked):
        """Mark the last gates on each ray and merge in one Rust call."""
        rust_n_gates = _can_use_rust_last_gates_merge(
            self._gate_excluded, fdata, n_gates, op, exclude_masked
        )
        if rust_n_gates is None:
            return False
        kernel = _rust_kernel("_gatefilter_last_gates_merge")
        if kernel is None:
            return False

        self._gate_excluded = kernel(self._gate_excluded, rust_n_gates, op)
        return True

    def _merge_transition(self, transition_data, trans_value, invert_marked, op, exclude_masked):
        """Mark transition rays and merge in one Rust call."""
        rust_trans_value = _can_use_rust_transition_merge(
            self._gate_excluded, transition_data, trans_value, op, exclude_masked
        )
        if rust_trans_value is None:
            return False
        kernel = _rust_kernel("_gatefilter_transition_merge")
        if kernel is None:
            return False
    ...
    def exclude_transition(self, trans_value=1, exclude_masked=True, op="or"):
        ...
        if self._radar.antenna_transition is not None:
            ...
            if self._merge_transition(
                transition_data, trans_value, False, op, exclude_masked
            ):
                return
    ...
    def exclude_below(
        ...
        comparator = "le" if inclusive else "lt"
        if self._merge_compare_threshold(
            fdata, value, comparator, False, op, exclude_masked
        ):
            return
        if inclusive:
            marked = fdata <= value
    ...
    def exclude_above(
        ...
        fdata = self._get_fdata(field)
        comparator = "ge" if inclusive else "gt"
        if self._merge_compare_threshold(
            fdata, value, comparator, False, op, exclude_masked
        ):
            return
    ...
    def exclude_above_toa(self, value, exclude_masked=True, op="or", inclusive=False):
        ...
        fdata = self._radar.gate_altitude["data"]
        comparator = "ge" if inclusive else "gt"
        if self._merge_compare_threshold(
            fdata, value, comparator, False, op, exclude_masked
        ):
    ...
    def exclude_inside(
        ...
        if v2 < v1:
            v1, v2 = (v2, v1)
        fdata = self._get_fdata(field)
        if self._merge_compare_interval(
            fdata, v1, v2, "inside", inclusive, False, op, exclude_masked
        ):
            return
        if inclusive:
    ...
    def exclude_outside(
        ...
        fdata = self._get_fdata(field)
        if self._merge_compare_interval(
            fdata, v1, v2, "outside", inclusive, False, op, exclude_masked
        ):
            return
        if inclusive:
            marked = (fdata <= v1) | (fdata >= v2)
    ...
    def exclude_equal(self, field, value, exclude_masked=True, op="or"):
        ...
        fdata = self._get_fdata(field)
        if self._merge_compare_threshold(fdata, value, "eq", False, op, exclude_masked):
            return
        marked = fdata == value
        return self._merge(marked, op, exclude_masked)
    ...
    def exclude_not_equal(self, field, value, exclude_masked=True, op="or"):
        ...
        fdata = self._get_fdata(field)
        if self._merge_compare_threshold(fdata, value, "ne", False, op, exclude_masked):
            return
        marked = fdata != value
        return self._merge(marked, op, exclude_masked)
    ...
    def exclude_invalid(self, field, exclude_masked=True, op="or"):
        ...
        """
        fdata = self._get_fdata(field)
        if self._merge_finite(fdata, op, exclude_masked):
            return
        marked = ~np.isfinite(fdata)
        return self._merge(marked, op, exclude_masked)
    ...
    def exclude_last_gates(self, field, n_gates=10, exclude_masked=True, op="or"):
        ...
        """
        fdata = self._get_fdata(field)
        if self._merge_last_gates(fdata, n_gates, op, exclude_masked):
            return
        marked = np.full(fdata.shape, False)
        marked[:, -n_gates:] = True
        return self._merge(marked, op, exclude_masked)
    ...
    def include_not_transition(self, trans_value=0, exclude_masked=True, op="and"):
        ...
        if self._radar.antenna_transition is None:
            ...
            transition_data = self._radar.antenna_transition["data"]
            if self._merge_transition(
                transition_data, trans_value, True, op, exclude_masked
            ):
                return
    ...
    def include_below(
        ...
        comparator = "le" if inclusive else "lt"
        if self._merge_compare_threshold(
            fdata, value, comparator, True, op, exclude_masked
        ):
            return
        if inclusive:
            marked = fdata <= value
    ...
    def include_inside(
        ...
        fdata = self._get_fdata(field)
        if self._merge_compare_interval(
            fdata, v1, v2, "inside", inclusive, True, op, exclude_masked
        ):
            return
        if inclusive:
            marked = (fdata >= v1) & (fdata <= v2)
    ...
    def include_outside(
        ...
        fdata = self._get_fdata(field)
        if self._merge_compare_interval(
            fdata, v1, v2, "outside", inclusive, True, op, exclude_masked
        ):
            return
        if inclusive:
            marked = (fdata <= v1) | (fdata >= v2)
    ...
    def include_equal(self, field, value, exclude_masked=True, op="and"):
        ...
        fdata = self._get_fdata(field)
        if self._merge_compare_threshold(fdata, value, "eq", True, op, exclude_masked):
            return
        marked = fdata == value
        return self._merge(~marked, op, exclude_masked)
    ...
    def include_not_equal(self, field, value, exclude_masked=True, op="and"):
        ...
        fdata = self._get_fdata(field)
        if self._merge_compare_threshold(fdata, value, "ne", True, op, exclude_masked):
            return
        marked = fdata != value
        return self._merge(~marked, op, exclude_masked)
