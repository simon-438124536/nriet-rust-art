...
def _can_use_rust_freq_band(freq):
    return (
        type(freq) is float
        or type(freq) is np.float64
        or (type(freq) is int and -1.0e20 <= freq <= 1.0e20)
    )


def _standardize_linear_rust(data, field_name, mx, mn):
    if field_name in ("relH", "KDP", "RhoHV"):
        return None
    if not (
        ...
        and np.all(np.abs(data) < 1.0e100)
    ):
        return None
    kernel = _rust_kernel("_echo_class_standardize_linear")
    if kernel is None:
        return None
    return kernel(data, float(mx), float(mn))
...
def _standardize(data, field_name, mx=None, mn=None):
    ...
    rust_field_std = _standardize_linear_rust(data, field_name, mx, mn)
    if rust_field_std is not None:
        return rust_field_std
...
def _assign_to_class_scan(
    ...
    """
    if t_vals is None:
        rust_hydroclass = _assign_to_class_scan_no_entropy_rust(
            fields_dict, mass_centers, var_names, weights
        )
        if rust_hydroclass is not None:
            return rust_hydroclass, None, None
...
def _assign_to_class_scan_no_entropy_rust(
    fields_dict,
    mass_centers,
    var_names,
    weights,
):
    kernel = _rust_kernel("_assign_to_class_scan_no_entropy")
    if kernel is None:
        return None

    mass_centers = np.asarray(mass_centers)
    weights = np.asarray(weights)
    if mass_centers.dtype != np.float64 or mass_centers.ndim != 2:
        return None
    if weights.dtype != np.float64 or weights.ndim != 1:
        return None

    nclasses, nvariables = mass_centers.shape
    if nclasses == 0 or nvariables == 0 or weights.shape != (nvariables,):
        return None
    if len(var_names) != nvariables:
        return None
    if not np.all(np.isfinite(mass_centers)):
        return None
    ...
    for var_name in var_names:
        try:
            field = fields_dict[var_name]
        except KeyError:
            return None

        if np.ma.isMaskedArray(field):
            data = np.asarray(np.ma.getdata(field))
        ...
        mask_arrays.append(np.asarray(mask, dtype=bool))

    data_stack = np.ascontiguousarray(np.stack(data_arrays, axis=0), dtype=np.float64)
    mask_stack = np.ascontiguousarray(np.stack(mask_arrays, axis=0), dtype=bool)
    if not np.all(np.isfinite(data_stack[~mask_stack])):
        return None

    hydroclass = kernel(
        data_stack,
        mask_stack,
        np.ascontiguousarray(mass_centers, dtype=np.float64),
        np.ascontiguousarray(weights, dtype=np.float64),
    )
    return np.ma.asarray(hydroclass, dtype=np.uint8)
...
def get_freq_band(freq):
    ...
    """
    kernel = _rust_kernel("_echo_class_get_freq_band")
    if kernel is not None and _can_use_rust_freq_band(freq):
        freq_band = kernel(float(freq))
