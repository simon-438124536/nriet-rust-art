...
def grid_displacement_pc(grid1, grid2, field, level, return_value="pixels"):
    ...
    yshift, xshift = _grid_displacement_peak(imageccorshift)
...
def _grid_displacement_peak(imageccorshift):
    rust_result = _grid_displacement_peak_rust(imageccorshift)
    if rust_result is not None:
        return rust_result

    row, col = imageccorshift.shape
...
def _grid_displacement_peak_rust(imageccorshift):
    if (
        type(imageccorshift) is not np.ndarray
        or imageccorshift.dtype != np.float64
        or imageccorshift.ndim != 2
        or imageccorshift.size == 0
        or not imageccorshift.flags.c_contiguous
    ):
        return None
    try:
        if not np.all(np.isfinite(imageccorshift)):
            return None
    except (TypeError, ValueError, FloatingPointError):
        return None

    kernel = _rust_kernel("_grid_displacement_peak_2d_f64")
    if kernel is None:
        return None
    try:
        yshift, xshift = kernel(imageccorshift)
    except Exception:
        return None
    return np.int64(yshift), np.int64(xshift)
