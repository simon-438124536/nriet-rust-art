...
def _can_use_rust_atwt2d(data2d, max_scale):
    return (
        type(data2d) is np.ndarray
        and data2d.ndim == 2
        and data2d.dtype == np.float64
        and data2d.flags.c_contiguous
        and data2d.flags.writeable
        and not np.ma.isMaskedArray(data2d)
        and isinstance(max_scale, (int, np.integer))
        and max_scale >= 0
    )


def _can_use_rust_label_classes(wt_sum, dbz_data, thresholds):
    numeric_scalar_types = (int, float, np.integer, np.floating)
    return (
        type(wt_sum) is np.ndarray
        and type(dbz_data) is np.ndarray
        and wt_sum.shape == dbz_data.shape
        and wt_sum.dtype == np.float64
        and dbz_data.dtype == np.float64
        and wt_sum.flags.c_contiguous
        and dbz_data.flags.c_contiguous
        and all(isinstance(value, numeric_scalar_types) for value in thresholds)
    )
...
def label_classes(
    ...
    wt_sum_data = np.ma.getdata(wt_sum)
    dbz_data_data = np.ma.getdata(dbz_data)
    kernel = _rust_kernel("_echo_class_wt_label_classes_f64")
    if (
        kernel is not None
        and _can_use_rust_label_classes(wt_sum_data, dbz_data_data, thresholds)
    ):
        try:
            rust_class = kernel(wt_sum_data, dbz_data_data, *map(float, thresholds))
        except (TypeError, ValueError, RuntimeError):
            rust_class = None
        if (
            type(rust_class) is np.ndarray
            and rust_class.shape == wt_sum_data.shape
            and rust_class.dtype == np.float64
            and rust_class.flags.c_contiguous
        ):
            return rust_class.astype(np.int32)
...
def atwt2d(data2d, max_scale=-1):
    ...
    kernel = _rust_kernel("_atwt2d")
    if kernel is not None and _can_use_rust_atwt2d(data2d, max_scale):
        return kernel(data2d, int(max_scale)), data2d
