...
def _can_use_rust_radial_mask(
    ...
    return (
        circular is True
        and type(mask_array) is np.ndarray
        and mask_array.ndim == 2
        and mask_array.dtype == np.float64
        and mask_array.shape[0] == mask_array.shape[1]
        and mask_array.flags.c_contiguous
        and mask_array.flags.writeable
        and all(np.isfinite(value) for value in scalar_values)
    )


def _assign_feature_radius_km_rust(field_bkg, val_for_max_rad, max_rad):
    try:
        scalar_values = (float(val_for_max_rad), float(max_rad))
    except (TypeError, ValueError, OverflowError):
        return None

    if not (
        type(field_bkg) is np.ndarray
        and field_bkg.dtype == np.float64
        and field_bkg.flags.c_contiguous
        and all(np.isfinite(value) for value in scalar_values)
        and np.isfinite(field_bkg).all()
    ):
        return None
    kernel = _rust_kernel("_assign_feature_radius_km")
    if kernel is None:
        return None
    return kernel(field_bkg, scalar_values[0], scalar_values[1])


def _classify_feature_array_rust(
    field,
    feature_array,
    core_array,
    NOSFCECHO,
    ...
    if not np.ma.isMaskedArray(field):
        return None
    if np.ma.isMaskedArray(feature_array):
        feature_mask_obj = feature_array.mask
        if (
            feature_mask_obj is not np.ma.nomask
            and isinstance(feature_mask_obj, np.ndarray)
            and not feature_mask_obj.flags.writeable
        ):
            return None
        feature_data = feature_array.data
        feature_mask = np.ma.getmaskarray(feature_array)
    else:
        feature_data = feature_array
        feature_mask = None
    if np.ma.isMaskedArray(core_array):
        core_data = core_array.data
        core_mask = np.ma.getmaskarray(core_array)
    else:
        core_data = core_array
        core_mask = np.zeros(np.shape(core_array), dtype=bool)

    field_data = field.data
    field_mask = np.ma.getmaskarray(field)
    if not (
        type(field_data) is np.ndarray
        and type(feature_data) is np.ndarray
        and type(core_data) is np.ndarray
        and type(core_mask) is np.ndarray
        and field_data.ndim > 0
        and field_data.dtype == np.float64
        and feature_data.dtype == np.float64
        and core_data.dtype == np.float64
        and core_mask.dtype == bool
        and field_data.shape == feature_data.shape
        and field_data.shape == field_mask.shape
        and field_data.shape == core_data.shape
        and field_data.shape == core_mask.shape
        and field_data.flags.c_contiguous
        and field_mask.flags.c_contiguous
        and feature_data.flags.c_contiguous
        and feature_data.flags.writeable
        and core_data.flags.c_contiguous
        and core_mask.flags.c_contiguous
    ):
        return None

    kernel = _rust_kernel("_classify_feature_array_f64")
    if kernel is None:
        return None
    kernel(
        field_data,
    ...
    if feature_mask is not None:
        feature_array.mask = np.zeros_like(feature_mask, dtype=bool)
    return feature_array


def _core_scalar_scheme_rust(
    field,
    field_bkg,
    max_diff,
    always_core_thres,
    ...
    try:
        scalar_values = (float(max_diff), float(always_core_thres), float(CORE))
    except (TypeError, ValueError, OverflowError):
        return None
    ...
    field_data = field.data
    field_mask = np.ma.getmaskarray(field)
    if np.ma.isMaskedArray(field_bkg):
        field_bkg_data = field_bkg.data
        field_bkg_mask = np.ma.getmaskarray(field_bkg)
    else:
        field_bkg_data = field_bkg
        field_bkg_mask = np.zeros(np.shape(field_bkg), dtype=bool)

    if not (
        type(field_data) is np.ndarray
        and type(field_bkg_data) is np.ndarray
        and type(field_bkg_mask) is np.ndarray
        and field_data.ndim > 0
        and field_data.dtype == np.float64
        and field_bkg_data.dtype == np.float64
        and field_bkg_mask.dtype == bool
        and field_data.shape == field_mask.shape
        and field_data.shape == field_bkg_data.shape
        and field_data.shape == field_bkg_mask.shape
        and field_data.flags.c_contiguous
        and field_mask.flags.c_contiguous
        and field_bkg_data.flags.c_contiguous
        and field_bkg_mask.flags.c_contiguous
        and all(np.isfinite(value) for value in scalar_values)
    ):
        return None

    kernel = _rust_kernel("_core_scalar_scheme_f64")
    if kernel is None:
        return None
    core_data = kernel(
        field_data,
    ...
    core_array = np.ma.masked_where(field_mask, core_data)
    core_array.set_fill_value(field.fill_value)
    return core_array
...
def create_radial_mask(
    ...
    kernel = _rust_kernel("_create_radial_mask_circular")
    if kernel is not None and _can_use_rust_radial_mask(
        mask_array,
        min_rad_km,
        max_rad_km,
        x_pixsize,
...
def core_scalar_scheme(
    ...
    rust_core = _core_scalar_scheme_rust(
        field, field_bkg, max_diff, always_core_thres, CORE, use_addition
    )
...
def assign_feature_radius_km(field_bkg, val_for_max_rad, max_rad=5):
    ...
    rust_radius = _assign_feature_radius_km_rust(field_bkg, val_for_max_rad, max_rad)
    if rust_radius is not None:
        return rust_radius
...
def classify_feature_array(
    ...
    rust_feature = _classify_feature_array_rust(
        field,
        feature_array,
        core_array,
        NOSFCECHO,
