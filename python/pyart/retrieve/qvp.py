...
def project_to_vertical(
    ...
    if interp_kind == "none":
        rust_data_out = _project_to_vertical_none_rust(
            data_in, data_height, grid_height
        )
        if rust_data_out is not _RUST_UNSUPPORTED:
            return rust_data_out
...
def find_rng_index(rng_vec, rng, rng_tol=0.0):
    ...
    """
    rust_index = _find_index_rust(rng_vec, rng, rng_tol)
    if rust_index is not _RUST_UNSUPPORTED:
        return rust_index
...
def find_nearest_gate(radar, lat, lon, latlon_tol=0.0005):
    ...
    """
    nearest = _find_nearest_gate_rust(
    ...
    if nearest is not _RUST_UNSUPPORTED:
        if nearest is None:
            warn(
...
def find_neighbour_gates(radar, azi, rng, delta_azi=None, delta_rng=None):
    ...
    rust_result = _find_neighbour_gates_rust(
        azimuths, ranges, azi, rng, delta_azi, delta_rng
    )
    if rust_result is not _RUST_UNSUPPORTED:
        return rust_result

    if delta_azi is None:
        inds_ray = np.ma.arange(azimuths.size)
    else:
        ...
        if azi_max > azi_min:
            inds_ray = np.where(
                np.logical_and(
                    azimuths < azi_max, azimuths > azi_min
                )
            )[0]
        else:
            inds_ray = np.where(
                np.logical_or(
                    azimuths > azi_min, azimuths < azi_max
                )
            )[0]
    if delta_rng is None:
        inds_rng = np.ma.arange(ranges.size)
    else:
        inds_rng = np.where(
            np.logical_and(
                ranges < rng + delta_rng,
                ranges > rng - delta_rng,
            )
        )[0]
...
def find_ang_index(ang_vec, ang, ang_tol=0.0):
    ...
    """
    rust_index = _find_index_rust(ang_vec, ang, ang_tol)
    if rust_index is not _RUST_UNSUPPORTED:
        return rust_index
...
def _find_index_rust(values, target, tolerance):
    kernel = _rust_kernel("_qvp_find_index_dense")
    if kernel is None or not _can_use_rust_find_index(values, target, tolerance):
        return _RUST_UNSUPPORTED
    return kernel(values, float(target), float(tolerance))


def _find_neighbour_gates_rust(azimuths, ranges, azi, rng, delta_azi, delta_rng):
    rust_values = _can_use_rust_find_neighbour_gates(
        azimuths, ranges, azi, rng, delta_azi, delta_rng
    )
    if rust_values is _RUST_UNSUPPORTED:
        return _RUST_UNSUPPORTED
    kernel = _rust_kernel("_qvp_find_neighbour_gates_dense")
    if kernel is None:
        return _RUST_UNSUPPORTED
    return kernel(azimuths, ranges, *rust_values)


def _find_nearest_gate_rust(gate_latitude, gate_longitude, lat, lon, latlon_tol):
    rust_values = _can_use_rust_find_nearest_gate(
        gate_latitude, gate_longitude, lat, lon, latlon_tol
    )
    if rust_values is _RUST_UNSUPPORTED:
        return _RUST_UNSUPPORTED
    kernel = _rust_kernel("_qvp_find_nearest_gate_dense")
    if kernel is None:
        return _RUST_UNSUPPORTED
    return kernel(gate_latitude, gate_longitude, *rust_values)


def _project_to_vertical_none_rust(data_in, data_height, grid_height):
    kernel = _rust_kernel("_qvp_project_to_vertical_none_dense_f64")
    if kernel is None or not _can_use_rust_project_to_vertical_none(
        data_in, data_height, grid_height
    ):
        return _RUST_UNSUPPORTED

    data, mask = kernel(data_in, data_height, grid_height)
    data_out = np.ma.masked_all(grid_height.size)
...
def _can_use_rust_find_index(values, target, tolerance):
    return (
...
def _can_use_rust_project_to_vertical_none(data_in, data_height, grid_height):
    return (
        type(data_in) is np.ndarray
        and type(data_height) is np.ndarray
        and type(grid_height) is np.ndarray
        and data_in.ndim == 1
        and data_height.ndim == 1
        and grid_height.ndim == 1
        and data_in.size > 0
        and data_height.shape == data_in.shape
        and grid_height.size >= 2
        and data_in.dtype == np.float64
        and data_height.dtype == np.float64
        and grid_height.dtype == np.float64
        and data_in.flags.c_contiguous
        and data_height.flags.c_contiguous
        and grid_height.flags.c_contiguous
        and np.all(np.isfinite(data_in))
        and np.all(np.isfinite(data_height))
        and np.all(np.isfinite(grid_height))
    )


def _finite_float_for_rust(value):
    if not isinstance(
...
def _can_use_rust_find_neighbour_gates(azimuths, ranges, azi, rng, delta_azi, delta_rng):
    if delta_azi is None or delta_rng is None:
        return _RUST_UNSUPPORTED
    azi = _finite_float_for_rust(azi)
    rng = _finite_float_for_rust(rng)
    delta_azi = _finite_float_for_rust(delta_azi)
    delta_rng = _finite_float_for_rust(delta_rng)
    if None in (azi, rng, delta_azi, delta_rng):
        return _RUST_UNSUPPORTED
    if not (
        type(azimuths) is np.ndarray
        and type(ranges) is np.ndarray
        and azimuths.ndim == 1
        and ranges.ndim == 1
        and azimuths.dtype == np.float64
        and ranges.dtype == np.float64
        and azimuths.flags.c_contiguous
        and ranges.flags.c_contiguous
        and np.all(np.isfinite(azimuths))
        and np.all(np.isfinite(ranges))
    ):
        return _RUST_UNSUPPORTED
    return azi, rng, delta_azi, delta_rng


def _can_use_rust_find_nearest_gate(gate_latitude, gate_longitude, lat, lon, latlon_tol):
    lat = _finite_float_for_rust(lat)
    lon = _finite_float_for_rust(lon)
    latlon_tol = _finite_float_for_rust(latlon_tol)
    if None in (lat, lon, latlon_tol):
        return _RUST_UNSUPPORTED
    if not (
        type(gate_latitude) is np.ndarray
        and type(gate_longitude) is np.ndarray
        and gate_latitude.ndim == 2
        and gate_longitude.ndim == 2
        and gate_latitude.shape == gate_longitude.shape
        and gate_latitude.dtype == np.float64
        and gate_longitude.dtype == np.float64
        and gate_latitude.flags.c_contiguous
        and gate_longitude.flags.c_contiguous
        and np.all(np.isfinite(gate_latitude))
        and np.all(np.isfinite(gate_longitude))
    ):
        return _RUST_UNSUPPORTED
    return lat, lon, latlon_tol
