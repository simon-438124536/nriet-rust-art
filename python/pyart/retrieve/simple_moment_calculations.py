...
def _can_use_rust_snr(refl, noisedBZ):
    return (
        type(refl) is np.ndarray
        and type(noisedBZ) is np.ndarray
        and refl.shape == noisedBZ.shape
        and refl.dtype == np.float64
        and noisedBZ.dtype == np.float64
        and refl.flags.c_contiguous
        and noisedBZ.flags.c_contiguous
    )


def _can_use_rust_compute_l_clamp(rhohv):
    return (
        type(rhohv) is np.ndarray
        and rhohv.dtype == np.float64
        and rhohv.flags.c_contiguous
        and rhohv.flags.writeable
    )


def _compute_l_rust(rhohv):
    if not (
        type(rhohv) is np.ndarray
        and rhohv.dtype == np.float64
        and rhohv.flags.c_contiguous
        and rhohv.flags.writeable
        and not np.ma.isMaskedArray(rhohv)
        and np.isfinite(rhohv).all()
        and (rhohv >= -1.0e300).all()
    ):
        return None
    kernel = _rust_kernel("_simple_moment_l_dense_f64")
    if kernel is None:
        return None
    l_data, l_mask = kernel(rhohv)
    return np.ma.array(l_data, mask=l_mask)


def _can_use_rust_cdr(rhohv, zdrdB):
    return (
        type(rhohv) is np.ndarray
        and type(zdrdB) is np.ndarray
        and rhohv.shape == zdrdB.shape
        and rhohv.dtype == np.float64
        and zdrdB.dtype == np.float64
        and rhohv.flags.c_contiguous
        and zdrdB.flags.c_contiguous
        and np.isfinite(rhohv).all()
        and np.isfinite(zdrdB).all()
        and (np.abs(zdrdB) <= 3000.0).all()
    )


def _cdr_rust(rhohv, zdrdB):
    kernel = _rust_kernel("_simple_moment_cdr_dense_f64")
    if kernel is None or not _can_use_rust_cdr(rhohv, zdrdB):
        return None
    cdr_data, cdr_mask = kernel(rhohv, zdrdB)
    return np.ma.array(cdr_data, mask=cdr_mask)


def _rust_tile_rows_f64(values, nrays):
    try:
        nrays_index = operator.index(nrays)
    except TypeError:
        return None
    if nrays_index < 0:
        return None
    ...
    if not (
        ...
        and type(values_data) is np.ndarray
        and type(values_mask) is np.ndarray
        and values_data.ndim == 1
        and values_mask.ndim == 1
        and values_data.shape == values_mask.shape
        and values_data.dtype == np.float64
        and values_mask.dtype == np.bool_
        and values_data.flags.c_contiguous
        and values_mask.flags.c_contiguous
    ):
        return None

    if values_mask.any():
        kernel = _rust_kernel("_simple_moment_tile_rows_masked_f64")
        if kernel is None:
            return None
        tiled_data, tiled_mask = kernel(values_data, values_mask, nrays_index)
        tiled = np.ma.array(tiled_data, mask=tiled_mask)
        tiled.set_fill_value(values.fill_value)
        return tiled

    kernel = _rust_kernel("_simple_moment_tile_rows_f64")
    if kernel is None:
        return None

    tiled_data = kernel(values_data, nrays_index)
    tiled = np.ma.array(
        tiled_data,
        mask=np.zeros((nrays_index, values_data.shape[0]), dtype=np.bool_),
    )
    tiled.set_fill_value(values.fill_value)
    return tiled
...
def compute_noisedBZ(nrays, noisedBZ_val, _range, ref_dist, noise_field=None):
    ...
    rust_tiled = _rust_tile_rows_f64(noisedBZ_vec, nrays)
    if rust_tiled is not None:
        noisedBZ["data"] = rust_tiled
    else:
...
def compute_snr(radar, refl_field=None, noise_field=None, snr_field=None):
    ...
    kernel = _rust_kernel("_simple_moment_snr_dense")
    if kernel is not None and _can_use_rust_snr(refl, noisedBZ):
        snr_data = kernel(refl, noisedBZ)
    else:
...
def compute_l(radar, rhohv_field=None, l_field=None):
    ...
    l_data = _compute_l_rust(rhohv)
    if l_data is None:
        kernel = _rust_kernel("_simple_moment_clamp_ge_f64")
        if kernel is not None and _can_use_rust_compute_l_clamp(rhohv):
            kernel(rhohv, 1.0, 0.9999)
        else:
...
def compute_cdr(radar, rhohv_field=None, zdr_field=None, cdr_field=None):
    ...
    cdr_data = _cdr_rust(rhohv, zdrdB)
    if cdr_data is None:
