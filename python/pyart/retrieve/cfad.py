...
def create_cfad(
    ...
    freq_norm = _normalize_cfad(freq, min_frac_thres)

    return freq_norm, height_edges, field_edges


def _normalize_cfad(freq, min_frac_thres):
    rust_result = _normalize_cfad_rust(freq, min_frac_thres)
...
def _normalize_cfad_rust(freq, min_frac_thres):
    args = _can_use_rust_normalize_cfad(freq, min_frac_thres)
    if args is None:
        return None
    kernel = _rust_kernel("_cfad_normalize_dense_f64")
    if kernel is None:
        return None
    data, mask = kernel(*args)
    return np.ma.masked_where(mask, data)


def _can_use_rust_normalize_cfad(freq, min_frac_thres):
    if not (
        type(freq) is np.ndarray
        and freq.ndim == 2
        and freq.dtype == np.float64
        and freq.flags.c_contiguous
        and freq.shape[0] > 0
        and np.isfinite(freq).all()
    ):
        return None
    if isinstance(min_frac_thres, (bool, np.bool_)):
        return None
    if not isinstance(min_frac_thres, (int, float, np.integer, np.floating)):
        return None
    min_frac_value = float(min_frac_thres)
    if not np.isfinite(min_frac_value):
        return None
    freq_sum = np.sum(freq, axis=1)
    if np.any(freq_sum <= 0.0):
        return None
    return freq, min_frac_value
