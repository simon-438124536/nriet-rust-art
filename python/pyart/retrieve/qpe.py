...
def _rust_default_qpe_coeff(name, freq_band, coeff_dict, default_coeff_dict):
    try:
        is_default_table = coeff_dict == default_coeff_dict
    except Exception:
        is_default_table = False
    if not is_default_table:
        return None
    kernel = _rust_kernel(name)
    if kernel is None:
        return None
    return kernel(freq_band)


def _default_coeff_rkdp_table():
    return {"S": (50.70, 0.8500), "C": (29.70, 0.8500), "X": (15.81, 0.7992)}


def _default_coeff_ra_table():
    return {"S": (3100.0, 1.03), "C": (250.0, 0.91), "X": (45.5, 0.83)}


def _strict_float_scalar(value):
...
def _zpoly_rust(refl):
    if not (
        type(refl) is np.ndarray
        and refl.dtype == np.float64
        and refl.flags.c_contiguous
        and np.isfinite(refl).all()
        and (np.abs(refl) <= 1000.0).all()
    ):
        return None
    kernel = _rust_kernel("_qpe_zpoly_dense_f64")
    if kernel is None:
        return None
    return np.ma.array(kernel(refl))


def _rain_rate_z_rust(refl, alpha, beta):
    alpha_value = _strict_float_scalar(alpha)
    beta_value = _strict_float_scalar(beta)
    ...
    if not (
        ...
        and refl.dtype == np.float64
        and refl.flags.c_contiguous
        and abs(alpha_value) <= 1.0e50
        and np.isfinite(refl).all()
        and (np.abs(refl) <= 1000.0).all()
    ):
        return None
    exponent = 0.1 * refl * beta_value
    if not (np.isfinite(exponent).all() and (np.abs(exponent) <= 250.0).all()):
        return None
    kernel = _rust_kernel("_qpe_rain_rate_z_dense_f64")
    if kernel is None:
        return None
    return np.ma.array(kernel(refl, alpha_value, beta_value))


def _rain_rate_kdp_rust(kdp, alpha, beta):
    alpha_value = _strict_float_scalar(alpha)
    beta_value = _strict_float_scalar(beta)
    if alpha_value is None or beta_value is None:
        return None
    if not (
        type(kdp) is np.ndarray
        and kdp.dtype == np.float64
        and kdp.flags.c_contiguous
        and abs(alpha_value) <= 1.0e50
        and np.isfinite(kdp).all()
        and (kdp >= 0.0).all()
        and (kdp <= 1.0e12).all()
    ):
        return None
    positive = kdp > 0.0
    if positive.any():
        exponents = np.log(kdp[positive]) * beta_value
        if not (np.isfinite(exponents).all() and (np.abs(exponents) <= 250.0).all()):
            return None
    elif beta_value < 0.0:
        return None
    kernel = _rust_kernel("_qpe_rain_rate_kdp_dense_f64")
    if kernel is None:
        return None
    return np.ma.array(kernel(kdp, alpha_value, beta_value))


def _rain_rate_a_rust(att, alpha, beta):
    alpha_value = _strict_float_scalar(alpha)
    beta_value = _strict_float_scalar(beta)
    if alpha_value is None or beta_value is None:
        return None
    if not (
        type(att) is np.ndarray
        and att.dtype == np.float64
        and att.flags.c_contiguous
        and abs(alpha_value) <= 1.0e50
    ...
    positive = att > 0.0
    if positive.any():
        exponents = np.log(att[positive]) * beta_value
        if not (np.isfinite(exponents).all() and (np.abs(exponents) <= 250.0).all()):
            return None
    elif beta_value < 0.0:
        return None
    kernel = _rust_kernel("_qpe_rain_rate_a_dense_f64")
    if kernel is None:
        return None
    return np.ma.array(kernel(att, alpha_value, beta_value))


def _blend_rain_rate_threshold_rust(rain_main, rain_secondary, thresh, thresh_max):
    thresh_value = _strict_float_scalar(thresh)
    if thresh_value is None or not isinstance(thresh_max, (bool, np.bool_)):
        return None
    if not (np.ma.isMaskedArray(rain_main) and np.ma.isMaskedArray(rain_secondary)):
        return None
    if np.ma.getmaskarray(rain_main).any() or np.ma.getmaskarray(rain_secondary).any():
        return None

    rain_main_data = rain_main.data
    rain_secondary_data = rain_secondary.data
    if not (
        type(rain_main_data) is np.ndarray
        and type(rain_secondary_data) is np.ndarray
        and rain_main_data.dtype == np.float64
        and rain_secondary_data.dtype == np.float64
        and rain_main_data.shape == rain_secondary_data.shape
        and rain_main_data.flags.c_contiguous
        and rain_secondary_data.flags.c_contiguous
        and rain_main_data.flags.writeable
        and np.isfinite(rain_main_data).all()
        and np.isfinite(rain_secondary_data).all()
    ):
        return None

    kernel = _rust_kernel("_qpe_threshold_blend_dense_f64")
    if kernel is None:
        return None
    kernel(rain_main_data, rain_secondary_data, thresh_value, bool(thresh_max))
    return True
...
def est_rain_rate_z(radar, alpha=0.0376, beta=0.6112, refl_field=None, rr_field=None):
    ...
    rr_data = _rain_rate_z_rust(refl, alpha, beta)
    if rr_data is None:
...
def est_rain_rate_kdp(radar, alpha=None, beta=None, kdp_field=None, rr_field=None):
    ...
    kdp[kdp < 0] = 0.0
    rr_data = _rain_rate_kdp_rust(kdp, alpha, beta)
    if rr_data is None:
...
def est_rain_rate_a(radar, alpha=None, beta=None, a_field=None, rr_field=None):
    ...
    rr_data = _rain_rate_a_rust(att, alpha, beta)
    if rr_data is None:
...
def est_rain_rate_zkdp(
    ...
    if not _blend_rain_rate_threshold_rust(
        rain_main["data"], rain_secondary["data"], thresh, thresh_max
    ):
        if thresh_max:
...
def _get_coeff_rkdp(freq):
    ...
    if (freq_band is not None) and (freq_band in coeff_rkdp_dict):
        rust_coeff = _rust_default_qpe_coeff(
            "_qpe_coeff_rkdp", freq_band, coeff_rkdp_dict, _default_coeff_rkdp_table()
        )
        if rust_coeff is not None:
            return rust_coeff
    ...
    rust_coeff = _rust_default_qpe_coeff(
        "_qpe_coeff_rkdp", freq_band_aux, coeff_rkdp_dict, _default_coeff_rkdp_table()
    )
    if rust_coeff is not None:
        return rust_coeff
...
def _get_coeff_ra(freq):
    ...
    if (freq_band is not None) and (freq_band in coeff_ra_dict):
        rust_coeff = _rust_default_qpe_coeff(
            "_qpe_coeff_ra", freq_band, coeff_ra_dict, _default_coeff_ra_table()
        )
        if rust_coeff is not None:
            return rust_coeff
    ...
    rust_coeff = _rust_default_qpe_coeff(
        "_qpe_coeff_ra", freq_band_aux, coeff_ra_dict, _default_coeff_ra_table()
    )
    if rust_coeff is not None:
        return rust_coeff
