...
CAPPI_RUST_MAX_VOLUME_GATES = 512 * 1024 * 1024
...
def create_cappi(
    ...
    height_idx, selected_gate_z = _cappi_height_index(z_3d, height)
...
def _cappi_height_index(z_3d, height):
    rust_result = _cappi_height_index_rust(z_3d, height)
    ...
    return height_idx, selected_gate_z


def _cappi_height_index_rust(z_3d, height):
    if (
        type(z_3d) is not np.ndarray
        or z_3d.dtype != np.float64
        or z_3d.ndim != 3
        or z_3d.shape[0] == 0
        or z_3d.size > CAPPI_RUST_MAX_VOLUME_GATES
        or not z_3d.flags.c_contiguous
    ):
        return None
    try:
        height = float(height)
    ...
    kernel = _rust_kernel("_cappi_height_index_f64")
    if kernel is None:
        return None
    try:
        return kernel(z_3d, height)
    except Exception:
        return None
