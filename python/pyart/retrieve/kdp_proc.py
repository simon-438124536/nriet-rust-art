...
def _forward_reverse_phidp(k, bcs, verbose=False):
    ...
    """
    phidp_f, phidp_r = _kdp_proc.forward_reverse_phidp(k, bcs)
...
def _parse_range_resolution(radar, check_uniform=True, atol=1.0, verbose=False):
    ...
    """
    rust_dr = _parse_range_resolution_rust(radar.range["data"], check_uniform, atol)
    if rust_dr is not None:
        dr = np.float64(rust_dr)
        if verbose:
...
def _parse_range_resolution_rust(ranges, check_uniform, atol):
    if check_uniform is not True:
        return None
    try:
        atol = float(atol)
    ...
    if not (
        ...
        return None
    kernel = _kdp_proc._rust_kernel("_kdp_range_resolution_uniform")
    if kernel is None:
        return None
    return kernel(ranges, atol)
