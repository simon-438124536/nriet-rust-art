...
def _is_rust_float64_compatible(value):
    if np.ma.isMaskedArray(value):
        return False
    if isinstance(value, np.ndarray):
        return type(value) is np.ndarray and value.dtype == np.float64
    if isinstance(value, np.generic):
        return value.dtype == np.float64
    return isinstance(value, (float, int)) and not isinstance(value, bool)


def _rust_broadcast_shape(*values):
    try:
        return np.broadcast_shapes(*(np.shape(value) for value in values))
    except ValueError:
        return None


def _can_use_rust_transform(*values, allow_scalar_result=True):
    shape = _rust_broadcast_shape(*values)
    if shape is None:
        return False, None
    if shape == () and not allow_scalar_result:
        return False, shape
    return all(_is_rust_float64_compatible(value) for value in values), shape


def _as_float64_array(value):
    return np.asarray(value, dtype=np.float64)


def _restore_scalar_result(result, shape):
    if shape == ():
...
def _can_use_rust_array64(*values):
    ...
    if shape is None:
        return False
    return all(type(value) is np.ndarray and value.dtype == np.float64 for value in values)


def _can_use_rust_edges_f32(values):
    return (
        ...
        and not np.ma.isMaskedArray(values)
    )
...
def antenna_to_cartesian(ranges, azimuths, elevations):
    ...
    """
    kernel = _rust_kernel("_antenna_to_cartesian")
    can_use_rust, shape = _can_use_rust_transform(ranges, azimuths, elevations)
    if kernel is not None and can_use_rust:
        result = kernel(
            _as_float64_array(ranges),
            _as_float64_array(azimuths),
            _as_float64_array(elevations),
        )
        return _restore_scalar_result(result, shape)
...
def cartesian_to_antenna(x, y, z):
    ...
    """
    kernel = _rust_kernel("_cartesian_to_antenna")
    can_use_rust, _ = _can_use_rust_transform(x, y, z, allow_scalar_result=False)
    if kernel is not None and can_use_rust:
        return kernel(_as_float64_array(x), _as_float64_array(y), _as_float64_array(z))
...
def _interpolate_range_edges(ranges):
    ...
    kernel = _rust_kernel("_interpolate_range_edges_f32")
    if kernel is not None and _can_use_rust_edges_f32(ranges):
...
def _interpolate_axes_edges(axes):
    ...
    kernel = _rust_kernel("_interpolate_axes_edges_f32")
    if kernel is not None and _can_use_rust_edges_f32(axes):
        return kernel(axes)
...
def geographic_to_cartesian_aeqd(lon, lat, lon_0, lat_0, R=6370997.0):
    ...
    kernel = _rust_kernel("_geographic_to_cartesian_aeqd")
    if kernel is not None and _can_use_rust_array64(lon, lat):
        return kernel(lon, lat, float(lon_0), float(lat_0), float(R))
...
def cartesian_to_geographic_aeqd(x, y, lon_0, lat_0, R=6370997.0):
    ...
    kernel = _rust_kernel("_cartesian_to_geographic_aeqd")
    if kernel is not None and _can_use_rust_array64(x, y):
        return kernel(x, y, float(lon_0), float(lat_0), float(R))
