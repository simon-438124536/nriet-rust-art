...
from .._rust_bridge import get_rust_module as _get_rust_module
...
def _can_use_rust_region_cost_inputs(
    nyq_vector, vels_slice_means, svels_slice_means, v_nyq_vel, nfeatures
):
    try:
        nfeatures_index = _operator.index(nfeatures)
    except TypeError:
        return None

    if nfeatures_index <= 0:
        return None
    if not (
        type(nyq_vector) is np.ndarray
        and type(vels_slice_means) is np.ndarray
        and type(svels_slice_means) is np.ndarray
        and nyq_vector.ndim == 1
        and vels_slice_means.ndim == 1
        and svels_slice_means.ndim == 1
        and nyq_vector.dtype == np.float64
        and vels_slice_means.dtype == np.float64
        and svels_slice_means.dtype == np.float64
        and nyq_vector.flags.c_contiguous
        and vels_slice_means.flags.c_contiguous
        and svels_slice_means.flags.c_contiguous
        and not np.ma.isMaskedArray(nyq_vector)
        and not np.ma.isMaskedArray(vels_slice_means)
        and not np.ma.isMaskedArray(svels_slice_means)
        and nfeatures_index <= nyq_vector.size
        and nfeatures_index <= vels_slice_means.size
        and nfeatures_index <= svels_slice_means.size
    ):
        return None

    if not isinstance(v_nyq_vel, (bool, int, float, np.bool_, np.integer, np.floating)):
        return None
    v_nyq_vel = float(v_nyq_vel)
    if (
        not np.isfinite(v_nyq_vel)
        or not np.isfinite(nyq_vector[:nfeatures_index]).all()
        or not np.isfinite(vels_slice_means[:nfeatures_index]).all()
        or not np.isfinite(svels_slice_means[:nfeatures_index]).all()
        or abs(v_nyq_vel) > 1.0e70
        or np.max(np.abs(nyq_vector[:nfeatures_index])) > 1.0e70
        or np.max(np.abs(vels_slice_means[:nfeatures_index])) > 1.0e70
        or np.max(np.abs(svels_slice_means[:nfeatures_index])) > 1.0e70
    ):
        return None

    return nfeatures_index


def _can_use_rust_sweep_interval_splits(nyquist, interval_splits, velocities):
    try:
        interval_splits_index = _operator.index(interval_splits)
    except TypeError:
        return None
    if interval_splits_index <= 0:
        return None
    if not isinstance(nyquist, (bool, int, float, np.bool_, np.integer, np.floating)):
        return None
    try:
    ...
    if interval_splits_index > intp_max:
        return None
    if not (
        type(velocities) is np.ndarray
        and velocities.ndim == 1
        and velocities.dtype == np.float64
        and velocities.flags.c_contiguous
        and not np.ma.isMaskedArray(velocities)
        and np.isfinite(velocities).all()
    ):
        return None
    interval = (2.0 * nyquist) / interval_splits_index
    ...
    if velocities.size:
        ...
        if max_vel > nyquist or min_vel < -nyquist:
            with np.errstate(over="ignore", invalid="ignore", divide="ignore"):
                add_start_float = np.ceil((max_vel - nyquist) / interval)
                add_end_float = np.ceil(-(min_vel + nyquist) / interval)
            if (
                not np.isfinite(add_start_float)
                or not np.isfinite(add_end_float)
                or add_start_float > intp_max
                or add_end_float > intp_max
            ):
                return None
            add_start = max(0, int(add_start_float))
            add_end = max(0, int(add_end_float))
        if interval_splits_index + 1 + add_start + add_end > intp_max:
            return None
    return nyquist, interval_splits_index


def _can_use_rust_edge_sum_and_count(index1, index2, vel1, vel2):
    return (
        type(index1) is np.ndarray
        and type(index2) is np.ndarray
        and type(vel1) is np.ndarray
        and type(vel2) is np.ndarray
        and index1.ndim == 1
        and index2.ndim == 1
        and vel1.ndim == 1
        and vel2.ndim == 1
        and index1.shape == index2.shape == vel1.shape == vel2.shape
        and index1.dtype == np.int32
        and index2.dtype == np.int32
        and vel1.dtype == np.float64
        and vel2.dtype == np.float64
        and index1.flags.c_contiguous
        and index2.flags.c_contiguous
        and vel1.flags.c_contiguous
        and vel2.flags.c_contiguous
        and not np.ma.isMaskedArray(index1)
        and not np.ma.isMaskedArray(index2)
        and not np.ma.isMaskedArray(vel1)
        and not np.ma.isMaskedArray(vel2)
        and np.isfinite(vel1).all()
        and np.isfinite(vel2).all()
    )
...
def _find_sweep_interval_splits(nyquist, interval_splits, velocities, nsweep):
    ...
    rust_inputs = _can_use_rust_sweep_interval_splits(
        nyquist, interval_splits, velocities
    )
    if rust_inputs is not None:
        kernel = _rust_kernel("_region_sweep_interval_splits")
        if kernel is not None:
            start, end, num, outside_nyquist = kernel(
                rust_inputs[0], rust_inputs[1], velocities
            )
            if outside_nyquist:
                msg = (
                    "Velocities outside of the Nyquist interval found in "
                    f"sweep {nsweep}."
                )
...
def _edge_sum_and_count(
    ...
    kernel = _rust_kernel("_region_edge_sum_and_count")
    if kernel is not None and _can_use_rust_edge_sum_and_count(
...
def _cost_function(
    ...
    nfeatures_index = _can_use_rust_region_cost_inputs(
        nyq_vector, vels_slice_means, svels_slice_means, v_nyq_vel, nfeatures
    )
    kernel = _rust_kernel("_region_cost_function")
    if kernel is not None and nfeatures_index is not None:
        return kernel(
            nyq_vector,
            vels_slice_means,
            svels_slice_means,
...
def _gradient(nyq_vector, vels_slice_means, svels_slice_means, v_nyq_vel, nfeatures):
    ...
    kernel = _rust_kernel("_region_gradient")
    if kernel is not None and nfeatures_index is not None:
        return kernel(
            nyq_vector,
            vels_slice_means,
