...
def _can_use_rust_smooth_and_trim(x, window_len, window):
    if window_len < 3:
        return False
    if not isinstance(window_len, (int, np.integer)):
        return False
    if window == "sg_smooth" and int(window_len) != 5:
        return False
    return (
        ...
        and x.size >= int(window_len)
        and not np.ma.isMaskedArray(x)
        and np.isfinite(x).all()
    )


def _can_use_rust_smooth_and_trim_scan(x, window_len, window):
    if window_len < 3:
        return False
    if not isinstance(window_len, (int, np.integer)):
        return False
    ...
    return (
        ...
        and x.dtype == np.float64
        and x.flags.c_contiguous
        and x.shape[1] >= int(window_len)
        and not np.ma.isMaskedArray(x)
        and np.isfinite(x).all()
    )


def _unwrap_masked_rust(lon, valid_count, centered):
    data = np.asarray(lon.data)
    mask = np.ma.getmaskarray(lon)
    if not (
    ...
    if not np.isfinite(data[~mask]).all():
        return None
    kernel = _rust_kernel("_phase_proc_unwrap_masked_degrees_f64")
    if kernel is None:
        return None
    try:
        x = kernel(data, mask)
    ...
    if not (
        ...
        and x.dtype == np.float64
        and x.ndim == 1
        and x.shape == (valid_count,)
        and x.flags.c_contiguous
    ):
        return None
    if centered:
        x = x - 360 * np.round(x.mean() / 360.0)
    return x


def _can_use_rust_det_sys_phase(ncp, rhv, phidp, last_ray_idx, ncp_lev, rhv_lev):
    if not (
        type(ncp) is np.ndarray
        and type(rhv) is np.ndarray
        and type(phidp) is np.ndarray
        and ncp.ndim == 2
        and rhv.ndim == 2
        and phidp.ndim == 2
        and ncp.dtype == np.float64
        and rhv.dtype == np.float64
        and phidp.dtype == np.float64
        and ncp.shape == rhv.shape == phidp.shape
        and ncp.flags.c_contiguous
        and rhv.flags.c_contiguous
        and phidp.flags.c_contiguous
        and not np.ma.isMaskedArray(ncp)
        and not np.ma.isMaskedArray(rhv)
        and not np.ma.isMaskedArray(phidp)
        and np.isfinite(ncp).all()
        and np.isfinite(rhv).all()
        and np.isfinite(phidp).all()
        and isinstance(last_ray_idx, (int, np.integer))
    ):
        return False
    last_ray_idx = int(last_ray_idx)
    if last_ray_idx < 0 or last_ray_idx >= ncp.shape[0]:
        return False
    if not (
        isinstance(ncp_lev, (int, float, np.integer, np.floating))
        and isinstance(rhv_lev, (int, float, np.integer, np.floating))
        and not isinstance(ncp_lev, (bool, np.bool_))
        and not isinstance(rhv_lev, (bool, np.bool_))
    ):
        return False
    try:
        ncp_lev = float(ncp_lev)
        rhv_lev = float(rhv_lev)
    except (TypeError, ValueError, OverflowError):
        return False
    return np.isfinite(ncp_lev) and np.isfinite(rhv_lev)


def _can_use_rust_det_sys_phase_gf(phidp, radar_meteo, last_ray_idx):
    if not (
        type(phidp) is np.ndarray
        and type(radar_meteo) is np.ndarray
        and phidp.ndim == 2
        and radar_meteo.ndim == 2
        and phidp.dtype == np.float64
        and radar_meteo.dtype == np.bool_
        and phidp.shape == radar_meteo.shape
        and phidp.flags.c_contiguous
        and radar_meteo.flags.c_contiguous
        and not np.ma.isMaskedArray(phidp)
        and not np.ma.isMaskedArray(radar_meteo)
        and np.isfinite(phidp).all()
        and isinstance(last_ray_idx, (int, np.integer))
    ):
        return False
    last_ray_idx = int(last_ray_idx)
    return 0 <= last_ray_idx < phidp.shape[0]
...
def _finite_real_scalar(value):
    ...
    if type(value) is np.ndarray:
        ...
        if value.dtype.kind not in "fiu":
            return None
        value = value.reshape(-1)[0] if value.shape == (1,) else value[()]
...
def _can_use_rust_fzl_index(fzl, ranges, elevation, radar_height):
    if not (
        type(ranges) is np.ndarray
        and ranges.ndim == 1
        and ranges.dtype == np.float64
        and ranges.flags.c_contiguous
        and not np.ma.isMaskedArray(ranges)
        and np.isfinite(ranges).all()
    ):
        return None
    fzl = _finite_real_scalar(fzl)
    elevation = _finite_real_scalar(elevation)
    radar_height = _finite_real_scalar(radar_height)
    if fzl is None or elevation is None or radar_height is None:
        return None
    return fzl, elevation, radar_height
...
def _det_sys_phase(ncp, rhv, phidp, last_ray_idx, ncp_lev=0.4, rhv_lev=0.6):
    ...
    kernel = _rust_kernel("_phase_proc_det_sys_phase_dense")
    if kernel is not None and _can_use_rust_det_sys_phase(
        ncp, rhv, phidp, last_ray_idx, ncp_lev, rhv_lev
    ):
        return kernel(
            ncp,
            rhv,
...
def fzl_index(fzl, ranges, elevation, radar_height):
    ...
    """
    kernel = _rust_kernel("_phase_proc_fzl_index_dense")
    scalar_args = _can_use_rust_fzl_index(fzl, ranges, elevation, radar_height)
    if kernel is not None and scalar_args is not None:
        fzl_scalar, elevation_scalar, radar_height_scalar = scalar_args
        return kernel(ranges, fzl_scalar, elevation_scalar, radar_height_scalar)
...
def unwrap_masked(lon, centered=False, copy=True):
    ...
    if lon.shape[0] < 2:
        return lon
    valid_count = int(lon.count())
    if valid_count < 2:
        return lon
    x = _unwrap_masked_rust(lon, valid_count, centered)
    if x is None:
        x = lon.compressed()
...
def smooth_and_trim(x, window_len=11, window="hanning"):
    ...
    kernel = _rust_kernel("_phase_proc_smooth_and_trim_f64")
    if kernel is not None and _can_use_rust_smooth_and_trim(x, window_len, window):
        return kernel(x, w / w.sum(), int(window_len))
...
def smooth_and_trim_scan(x, window_len=11, window="hanning"):
    ...
    weights = w / w.sum()
    kernel = _rust_kernel("_phase_proc_smooth_and_trim_scan_f64")
    if kernel is not None and _can_use_rust_smooth_and_trim_scan(x, window_len, window):
        try:
            return kernel(x, weights, int(window_len))
        except (TypeError, ValueError, FloatingPointError):
            pass

    y = convolve1d(x, weights, axis=1)
...
def _det_sys_phase_gf(phidp, last_ray_idx, radar_meteo):
    """Determine the system phase, see :py:func:`det_sys_phase`."""
    kernel = _rust_kernel("_phase_proc_det_sys_phase_gf_dense")
    if kernel is not None and _can_use_rust_det_sys_phase_gf(
        phidp, radar_meteo, last_ray_idx
    ):
        return kernel(phidp, radar_meteo, np.hanning(9) / np.hanning(9).sum(), int(last_ray_idx))
