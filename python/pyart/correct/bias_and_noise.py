...
def _can_use_rust_first_mask(data, noise_threshold):
    if not (
        ...
        return False
    if isinstance(noise_threshold, np.ndarray):
        if (
            type(noise_threshold) is not np.ndarray
            or noise_threshold.ndim != 0
            or not (
                np.issubdtype(noise_threshold.dtype, np.number)
                or np.issubdtype(noise_threshold.dtype, np.bool_)
            )
            or np.issubdtype(noise_threshold.dtype, np.complexfloating)
        ):
            return False
...
def _can_use_rust_cloud_threshold(data, n_avg, nffts):
    try:
        n_avg_value = float(n_avg)
        nffts_index = operator.index(nffts)
    except (TypeError, ValueError, OverflowError):
        return None
    ...
    if not (
        ...
        and data.flags.c_contiguous
        and not np.ma.isMaskedArray(data)
        and 0 <= nffts_index <= data.size
        and np.isfinite(n_avg_value)
        and 0.0 <= n_avg_value <= 1.0e12
        and np.isfinite(data).all()
        and (np.abs(data) <= 1000.0).all()
    ):
        return None
    return n_avg_value, nffts_index


def _can_use_rust_cloud_mask_4x4(mask1, data, counts_threshold):
    if isinstance(counts_threshold, (bool, np.bool_)):
        return None
    try:
        counts_threshold = operator.index(counts_threshold)
    except (TypeError, ValueError, OverflowError):
        return None

    if not (
        type(mask1) is np.ndarray
        and mask1.ndim == 2
        and mask1.dtype == np.int16
        and mask1.flags.c_contiguous
        and not np.ma.isMaskedArray(mask1)
        and type(data) is np.ndarray
        and data.shape == mask1.shape
        and data.flags.c_contiguous
        and 0 <= counts_threshold <= 16
    ):
        return None
    if mask1.size and not np.logical_or(mask1 == 0, mask1 == 1).all():
        return None
    return counts_threshold


def _cloud_mask_4x4_count(mask1, data, counts_threshold):
    counts_threshold_index = _can_use_rust_cloud_mask_4x4(
        mask1, data, counts_threshold
    )
    kernel = _rust_kernel("_cloud_mask_4x4_count_i16")
    if counts_threshold_index is not None and kernel is not None:
        return kernel(mask1, counts_threshold_index)
...
def _can_use_rust_correct_noise_rhohv(urhohv, snrdB_h, zdrdB, nh, nv):
    arrays = (urhohv, snrdB_h, zdrdB, nh, nv)
    if not (
        all(type(array) is np.ndarray for array in arrays)
        and all(array.dtype == np.float64 for array in arrays)
        and all(array.flags.c_contiguous for array in arrays)
        and all(not np.ma.isMaskedArray(array) for array in arrays)
        and all(array.shape == urhohv.shape for array in arrays[1:])
        and all(np.isfinite(array).all() for array in arrays)
        and (np.abs(urhohv) <= 1.0e6).all()
        and (np.abs(snrdB_h) <= 300.0).all()
        and (np.abs(zdrdB) <= 300.0).all()
        and (np.abs(nh) <= 300.0).all()
        and (np.abs(nv) <= 300.0).all()
        and (np.abs(nh - nv) <= 300.0).all()
    ):
        return False
    return True


def _correct_noise_rhohv_rust(urhohv, snrdB_h, zdrdB, nh, nv):
    if not _can_use_rust_correct_noise_rhohv(urhohv, snrdB_h, zdrdB, nh, nv):
        return None
    kernel = _rust_kernel("_correct_noise_rhohv_dense_f64")
    if kernel is None:
        return None
    return np.ma.array(kernel(urhohv, snrdB_h, zdrdB, nh, nv))
...
def correct_noise_rhohv(
    ...
    rhohv_data = _correct_noise_rhohv_rust(urhohv, snrdB_h, zdrdB, nh, nv)
    if rhohv_data is None:
...
def calc_cloud_mask(
    ...
    mask2 = _cloud_mask_4x4_count(mask1, data, counts_threshold)
...
def cloud_threshold(data, n_avg=1.0, nffts=None):
    ...
    kernel = _rust_kernel("_cloud_threshold_f64")
    scalar_args = _can_use_rust_cloud_threshold(data, n_avg, nffts)
    if kernel is not None and scalar_args is not None:
        return np.float64(kernel(data, scalar_args[0], scalar_args[1]))
...
def _first_mask(data, noise_threshold):
    kernel = _rust_kernel("_first_mask_f64")
    if kernel is not None and _can_use_rust_first_mask(data, noise_threshold):
        return kernel(data, float(noise_threshold))
