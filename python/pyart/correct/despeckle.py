...
def _can_use_rust_check_for_360(az, delta):
    if not (
        type(az) is np.ndarray
        and az.ndim == 1
        and az.dtype == np.float64
        and az.flags.c_contiguous
        and az.size > 0
        and not np.ma.isMaskedArray(az)
        and np.isfinite(az).all()
    ):
        return False
    if not isinstance(delta, (bool, int, float, np.bool_, np.integer, np.floating)):
        return False
    return np.isfinite(float(delta))
...
def _check_for_360(az, delta):
    ...
    """
    kernel = _rust_kernel("_despeckle_check_for_360")
    if kernel is not None and _can_use_rust_check_for_360(az, delta):
        return kernel(az, float(delta))
