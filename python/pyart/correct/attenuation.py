...
def get_mask_fzl(
    ...
    if temp_ref == "fixed_fzl":
        ...
        if temp_field in radar.fields:
            ...
            end_gate_arr = _end_gate_arr_from_excluded_mask(
                gatefilter.gate_excluded, radar.nrays, radar.ngates
            )
...
def _end_gate_arr_from_excluded_mask(gate_excluded, nrays, ngates):
    kernel = _rust_kernel("_attenuation_end_gate_from_excluded_mask")
    if kernel is not None and _can_use_rust_end_gate_mask(
        gate_excluded, nrays, ngates
    ):
        try:
            end_gate_arr = kernel(gate_excluded)
        except (TypeError, ValueError, RuntimeError):
            end_gate_arr = None
        if (
            type(end_gate_arr) is np.ndarray
            and end_gate_arr.ndim == 1
            and end_gate_arr.shape == (nrays,)
            and end_gate_arr.dtype == np.int32
            and end_gate_arr.flags.c_contiguous
        ):
            return end_gate_arr

    end_gate_arr = np.zeros(nrays, dtype="int32")
    for ray in range(nrays):
        ind_rng = np.where(gate_excluded[ray, :] == 1)[0]
        if len(ind_rng) > 0:
            ...
            end_gate_arr[ray] = ngates - 1
    return end_gate_arr


def _can_use_rust_end_gate_mask(gate_excluded, nrays, ngates):
    return (
        type(gate_excluded) is np.ndarray
        and gate_excluded.ndim == 2
        and gate_excluded.shape == (nrays, ngates)
...
def _prepare_phidp(phidp, mask_fzl):
    ...
    """
    phidp_data = np.ma.getdata(phidp)
    phidp_mask = np.ma.getmaskarray(phidp)
    kernel = _rust_kernel("_attenuation_prepare_phidp_dense")
    if kernel is not None and _can_use_rust_prepare_phidp(
        phidp_data, phidp_mask, mask_fzl
    ):
        return kernel(phidp_data, phidp_mask, mask_fzl)
...
def _can_use_rust_prepare_phidp(phidp_data, phidp_mask, mask_fzl):
    return (
        type(phidp_data) is np.ndarray
        and type(phidp_mask) is np.ndarray
        and type(mask_fzl) is np.ndarray
        and phidp_data.ndim == 2
        and phidp_mask.ndim == 2
        and mask_fzl.ndim == 2
        and phidp_data.shape == phidp_mask.shape
        and phidp_data.shape == mask_fzl.shape
        and phidp_data.dtype == np.float64
        and phidp_mask.dtype == np.bool_
        and mask_fzl.dtype == np.bool_
        and phidp_data.flags.c_contiguous
        and phidp_mask.flags.c_contiguous
        and mask_fzl.flags.c_contiguous
    )


def _rust_default_attenuation_params(name, freq_band, param_dict, default_param_dict):
    try:
        is_default_table = param_dict == default_param_dict
    except Exception:
...
def _default_param_attzphi_table():
    return {
        "S": (0.02, 0.64884, 0.15917, 1.0804),
        "C": (0.08, 0.64884, 0.3, 1.0804),
        "X": (0.31916, 0.64884, 0.15917, 1.0804),
    }


def _default_param_attphilinear_table():
    return {"S": (0.04, 0.004), "C": (0.08, 0.03), "X": (0.28, 0.04)}
...
def _get_param_attzphi(freq):
    ...
    if (freq_band is not None) and (freq_band in param_att_dict):
        rust_params = _rust_default_attenuation_params(
            "_attenuation_param_attzphi",
            freq_band,
            param_att_dict,
            _default_param_attzphi_table(),
        )
...
def _get_param_attphilinear(freq):
    ...
    if (freq_band is not None) and (freq_band in param_att_dict):
        rust_params = _rust_default_attenuation_params(
            "_attenuation_param_attphilinear",
            freq_band,
            param_att_dict,
            _default_param_attphilinear_table(),
        )
        if rust_params is not None:
            return rust_params
        return param_att_dict[freq_band]
