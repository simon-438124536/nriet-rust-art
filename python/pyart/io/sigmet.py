...
SIGMET_TIME_ORDER_RUST_MAX_RAYS = 1024 * 1024
...
def _first_metadata_field(metadata):
    """Return the first metadata field name using Python 3 dict view semantics."""
...
def _is_time_ordered_by_reversal(data, metadata, rays_per_sweep):
    ...
    rust_result = _sigmet_time_ordered_rust(
        "_sigmet_time_ordered_by_reversal_i32", ref_time, rays_per_sweep
    )
...
def _is_time_ordered_by_roll(data, metadata, rays_per_sweep):
    ...
    if "XHDR" in data:
        ...
        ref_time = metadata[_first_metadata_field(metadata)]["time"].astype("int32")
    rust_result = _sigmet_time_ordered_rust(
        "_sigmet_time_ordered_by_roll_i32", ref_time, rays_per_sweep
    )
    if rust_result is not None:
        return rust_result
    start = 0
...
def _is_time_ordered_by_reverse_roll(data, metadata, rays_per_sweep):
    ...
    if "XHDR" in data:
        ...
        ref_time = metadata[_first_metadata_field(metadata)]["time"].astype("int32")
    rust_result = _sigmet_time_ordered_rust(
        "_sigmet_time_ordered_by_reverse_roll_i32", ref_time, rays_per_sweep
    )
    if rust_result is not None:
        return rust_result
    start = 0
...
def _sigmet_time_ordered_rust(kernel_name, ref_time, rays_per_sweep):
    if (
        type(ref_time) is not np.ndarray
        or type(rays_per_sweep) is not np.ndarray
        or ref_time.dtype != np.int32
        or rays_per_sweep.dtype != np.int64
        or ref_time.ndim != 1
        or rays_per_sweep.ndim != 1
        or ref_time.size > SIGMET_TIME_ORDER_RUST_MAX_RAYS
        or rays_per_sweep.size > SIGMET_TIME_ORDER_RUST_MAX_RAYS
        or not ref_time.flags.c_contiguous
        or not rays_per_sweep.flags.c_contiguous
    ):
        return None
    if rays_per_sweep.size and np.any(rays_per_sweep < 0):
        return None
    ...
    if kernel is None:
        return None
    try:
        return bool(kernel(ref_time, rays_per_sweep))
    except Exception:
        return None


def _sigmet_time_order_index_rust(kernel_name, ref_time, rays_per_sweep):
    if (
        type(ref_time) is not np.ndarray
        or type(rays_per_sweep) is not np.ndarray
        or ref_time.dtype != np.int32
    ...
    if kernel is None:
        return None
    try:
        order = kernel(ref_time, rays_per_sweep)
    except Exception:
        return None
    if (
        type(order) is not np.ndarray
        or order.dtype != np.int64
        or order.ndim != 1
        or not order.flags.c_contiguous
    ):
        return None
    if order.shape[0] != ref_time.shape[0]:
        return None
    if order.size and (order.min() < 0 or order.max() >= order.shape[0]):
        return None
    if not np.array_equal(np.sort(order), np.arange(order.shape[0], dtype=np.int64)):
        return None
    return order


def _try_apply_sigmet_time_order(data, metadata, order):
    identity = np.arange(order.shape[0], dtype=np.int64)
    if np.array_equal(order, identity):
        return True
    if not _can_apply_sigmet_time_order(data, metadata, order.shape[0]):
        return False
    ...
    for field in data.keys():
        ...
        for key in field_metadata.keys():
            field_metadata[key][...] = field_metadata[key][order]
    return True


def _can_apply_sigmet_time_order(data, metadata, nrays):
    for field in data.keys():
        if field not in metadata:
            return False
        if not _sigmet_orderable_array(data[field], nrays):
            return False
        for value in metadata[field].values():
            if not _sigmet_orderable_array(value, nrays):
                return False
    return True


def _sigmet_orderable_array(value, nrays):
    return (
        type(value) is np.ndarray
        and value.ndim >= 1
        and value.shape[0] == nrays
...
def _time_order_data_and_metadata_roll(data, metadata, rays_per_sweep):
    ...
    rust_order = _sigmet_time_order_index_rust(
        "_sigmet_time_order_roll_index_i32", ref_time, rays_per_sweep
    )
    if rust_order is not None and _try_apply_sigmet_time_order(data, metadata, rust_order):
        return
...
def _time_order_data_and_metadata_reverse(data, metadata, rays_per_sweep):
    ...
    rust_order = _sigmet_time_order_index_rust(
        "_sigmet_time_order_reverse_index_i32", ref_time, rays_per_sweep
    )
    if rust_order is not None and _try_apply_sigmet_time_order(data, metadata, rust_order):
        return
...
def _time_order_data_and_metadata_full(data, metadata, rays_per_sweep):
    ...
    rust_order = _sigmet_time_order_index_rust(
        "_sigmet_time_order_full_index_i32", ref_time, rays_per_sweep
    )
    if rust_order is not None and _try_apply_sigmet_time_order(data, metadata, rust_order):
        return
