...
_RUST_RLE8_MAX_OUTPUT_BYTES = 512 * 1024 * 1024
...
def _decode_rle8(compr_data, key, decompr_size):
    ...
    kernel = _rust_kernel("_mdv_decode_rle8")
    rust_args = _can_use_rust_decode_rle8(compr_data, key, decompr_size)
    if kernel is not None and rust_args is not None:
        key, decompr_size = rust_args
        return kernel(compr_data, key, decompr_size)
...
def _can_use_rust_decode_rle8(compr_data, key, decompr_size):
    if not isinstance(compr_data, bytes):
        return None

    if isinstance(key, (bool, np.bool_)) or isinstance(decompr_size, (bool, np.bool_)):
        return None
    try:
        key = operator.index(key)
        decompr_size = operator.index(decompr_size)
    except TypeError:
        return None
    if key < 0 or key > 255 or decompr_size < 0:
        return None
    if decompr_size > _RUST_RLE8_MAX_OUTPUT_BYTES:
        return None

    data_ptr = 0
    out_ptr = 0
    data_len = len(compr_data)
    saw_run = False
    while data_ptr != data_len:
        value = compr_data[data_ptr]
        if value != key:
            out_ptr += 1
            data_ptr += 1
        else:
            saw_run = True
            if data_ptr + 2 >= data_len:
                return None
            out_ptr += compr_data[data_ptr + 1]
            data_ptr += 3
        if out_ptr > decompr_size:
            return None
        if saw_run and out_ptr > 255:
            return None

    if out_ptr != decompr_size:
        return None
    return key, decompr_size
