...
def _get_rgb_values(data, vmin, vmax, color_levels, cmap, transpbg, op):
    ...
    if color_levels is None:
        color_levels = 255
    rust_result = _get_rgb_values_rust(data, vmin, vmax, color_levels, cmap, transpbg, op)
...
def _get_rgb_values_rust(data, vmin, vmax, color_levels, cmap, transpbg, op):
    args = _can_use_rust_rgb_values(data, vmin, vmax, color_levels, transpbg, op)
    if args is None:
        return None
    data, vmin, vmax, color_levels, transpbg, op = args
    kernel = _rust_kernel("_geotiff_rgb_values_f64")
    if kernel is None:
        return None
    cmap = plt.get_cmap(cmap)
    rgba_lut = np.asarray(cmap(np.arange(256)), dtype=np.float64)
    try:
        rarr, garr, barr, aarr, has_nan = kernel(
            data, rgba_lut, vmin, vmax, color_levels, bool(transpbg), op
        )
    except Exception:
        return None
    if not has_nan:
        rarr = rarr.astype(np.int64)
        garr = garr.astype(np.int64)
        barr = barr.astype(np.int64)
    return rarr, garr, barr, aarr


def _can_use_rust_rgb_values(data, vmin, vmax, color_levels, transpbg, op):
    if not (type(data) is np.ndarray and data.dtype == np.float64):
        return None
    if data.ndim != 2 or not data.flags.c_contiguous:
        return None
    if np.ma.isMaskedArray(data):
        return None
    if isinstance(transpbg, (bool, np.bool_)) is False:
        return None
    if isinstance(color_levels, (bool, np.bool_)):
        return None
    try:
        vmin = float(vmin)
        vmax = float(vmax)
        color_levels = float(color_levels)
        op = float(op)
    ...
    if not (
        ...
        and np.isfinite(color_levels)
        and np.isfinite(op)
    ):
        return None
    if vmax == vmin:
        return None
    alpha = np.round(op * 255.0)
    if not np.isfinite(alpha) or alpha < np.iinfo(np.int64).min or alpha > np.iinfo(np.int64).max:
        return None
    return data, vmin, vmax, color_levels, bool(transpbg), op
