...
def _unpack_variable_gate_field_dic(dic, shape, ray_n_gates, ray_start_index):
    ...
    fdata = dic["data"]
    if _unpack_variable_gate_field_dic_rust(dic, fdata, shape, ray_n_gates, ray_start_index):
        return
...
def _unpack_variable_gate_field_dic_rust(
    dic, fdata, shape, ray_n_gates, ray_start_index
):
    args = _can_use_rust_unpack_variable_gate(
        fdata, shape, ray_n_gates, ray_start_index
    )
    if args is None:
        return False
    kernel = _rust_kernel("_cfradial_unpack_variable_gate_dense")
    if kernel is None:
        return False

    shape, gates, starts = args
    data = np.ma.masked_all(shape, dtype=fdata.dtype)
    mask = data.mask
    if type(mask) is not np.ndarray:
        return False
    kernel(fdata, data.data, mask, gates, starts)
    dic["data"] = data
    return True


def _can_use_rust_unpack_variable_gate(fdata, shape, ray_n_gates, ray_start_index):
    if not (
        type(fdata) is np.ndarray
        and fdata.ndim == 1
        and fdata.flags.c_contiguous
        and fdata.dtype.kind in "biufc"
    ):
        return None
    try:
        shape = tuple(_index_no_bool(value) for value in shape)
    except TypeError:
        return None
    if len(shape) != 2 or shape[0] < 0 or shape[1] < 0:
        return None
    if not (
        type(ray_n_gates) is np.ndarray
        and type(ray_start_index) is np.ndarray
        and ray_n_gates.ndim == 1
        and ray_start_index.ndim == 1
        and ray_n_gates.flags.c_contiguous
        and ray_start_index.flags.c_contiguous
        and ray_n_gates.dtype.kind in "iu"
        and ray_start_index.dtype.kind in "iu"
        and ray_n_gates.dtype.kind != "b"
        and ray_start_index.dtype.kind != "b"
    ):
        return None
    if ray_n_gates.size != shape[0] or ray_start_index.size != shape[0]:
        return None
    if _unsigned_exceeds_i64(ray_n_gates) or _unsigned_exceeds_i64(ray_start_index):
        return None

    gates = np.ascontiguousarray(ray_n_gates, dtype=np.int64)
    starts = np.ascontiguousarray(ray_start_index, dtype=np.int64)
    if gates.size == 0:
        return shape, gates, starts
    if np.any(gates < 0) or np.any(starts < 0):
        return None
    if np.any(gates > shape[1]) or np.any(starts > fdata.size):
        return None
    if np.any(gates > (fdata.size - starts)):
        return None
    return shape, gates, starts


def _index_no_bool(value):
    if isinstance(value, (bool, np.bool_)):
        raise TypeError("boolean shape values are not native-owned")
    return operator.index(value)


def _unsigned_exceeds_i64(array):
    if array.dtype.kind != "u" or array.size == 0:
        return False
    return np.max(array) > np.iinfo(np.int64).max
