...
def _get_moment_data(moment_var, index, ngates):
    ...
    rust_result = _get_moment_data_rust(moment_var, raw_moment_data)
...
def _get_moment_data_rust(moment_var, raw_moment_data):
    args = _can_use_rust_moment_data(moment_var, raw_moment_data)
    if args is None:
        return None
    kernel_name, raw_moment_data, scale, add_offset = args
    kernel = _rust_kernel(kernel_name)
    if kernel is None:
        return None
    data, mask = kernel(raw_moment_data, scale, add_offset)
    return np.ma.masked_array(data, mask=mask)


def _can_use_rust_moment_data(moment_var, raw_moment_data):
    if not (
        type(raw_moment_data) is np.ndarray
        and raw_moment_data.ndim == 2
        and raw_moment_data.flags.c_contiguous
        and raw_moment_data.dtype
        in (
            np.dtype(np.uint8),
            np.dtype(np.uint16),
            np.dtype(np.int8),
        ...
        return None
    attrs = moment_var.ncattrs()
    scale = getattr(moment_var, "scale_factor", 1.0) if "scale_factor" in attrs else 1.0
    add_offset = getattr(moment_var, "add_offset", 0.0) if "add_offset" in attrs else 0.0
    if np.result_type(raw_moment_data.dtype, scale, add_offset) != np.dtype(np.float64):
        return None
    scale = _finite_float_attr(scale)
    add_offset = _finite_float_attr(add_offset)
    if scale is None or add_offset is None:
        return None
    kernel_name = {
        np.dtype(np.uint8): "_nexrad_cdm_moment_u8",
        np.dtype(np.uint16): "_nexrad_cdm_moment_u16",
        np.dtype(np.int8): "_nexrad_cdm_moment_i8",
        np.dtype(np.int16): "_nexrad_cdm_moment_i16",
        np.dtype(np.float32): "_nexrad_cdm_moment_f32",
        np.dtype(np.float64): "_nexrad_cdm_moment_f64",
    }[raw_moment_data.dtype]
    return kernel_name, raw_moment_data, scale, add_offset
