...
class ChlFile:
    ...
    def _extract_fields(self):
        ...
        rust_fields = _extract_integer_fields_rust(self)
        all_data = None
        for i, field_num in enumerate(self._field_nums):
            if rust_fields is not None and field_num in rust_fields:
                self.fields[field_num] = rust_fields[field_num]
                continue
...
def _extract_integer_fields_rust(chl_file):
    args = _can_use_rust_chl_integer_fields(chl_file)
    if args is None:
        return None
    kernel = _rust_kernel("_chl_extract_integer_fields")
    if kernel is None:
        return None

    raw_data, ngates, field_nums, formats, dat_factors, dat_biases, fld_factors = args
    rust_result = kernel(
        raw_data,
        ngates,
        field_nums,
    ...
    fields = {}
    for field_num, data, mask, format_code in rust_result:
        mask_value = mask if mask.any() else False
        fdata = np.ma.masked_array(data, mask=mask_value)
        fdata._fill_value = np.array(0, dtype=DATA_FORMAT[format_code])
        fields[field_num] = fdata
    return fields


def _can_use_rust_chl_integer_fields(chl_file):
    if type(chl_file._dstring) is not bytes:
        return None
    try:
        ngates = int(chl_file.ngates)
    ...
    try:
        field_nums = [int(field_num) for field_num in chl_file._field_nums]
        formats = [int(chl_file.field_info[field_num]["format"]) for field_num in field_nums]
    except (TypeError, ValueError, KeyError):
        return None
    if len(field_nums) == 0 or len(field_nums) != len(formats):
        return None
    if any(format_code not in (0, 1, 2, 3) for format_code in formats):
        return None

    try:
        dtype = np.dtype(chl_file._dtype)
    except TypeError:
        return None
    if dtype.names is None:
        return None

    record_size = sum(np.dtype(DATA_FORMAT[format_code]).itemsize for format_code in formats)
    if dtype.itemsize != record_size:
        return None

    raw_len = len(chl_file._dstring)
    if record_size <= 0 or raw_len % record_size != 0:
        return None
    if (raw_len // record_size) % ngates != 0:
        return None

    dat_factors = []
    dat_biases = []
    fld_factors = []
    has_integer_field = False
    for field_num, format_code in zip(field_nums, formats):
        if format_code == 2:
            dat_factors.append(0.0)
            dat_biases.append(0.0)
            fld_factors.append(1.0)
            continue

        has_integer_field = True
        try:
            dat_factor = float(chl_file.field_info[field_num]["dat_factor"])
            dat_bias = float(chl_file.field_info[field_num]["dat_bias"])
            fld_factor = float(chl_file.field_info[field_num]["fld_factor"])
        except (TypeError, ValueError, KeyError):
            return None
        if not _safe_chl_integer_scale(format_code, dat_factor, dat_bias, fld_factor):
            return None
        dat_factors.append(dat_factor)
        dat_biases.append(dat_bias)
        fld_factors.append(fld_factor)

    if not has_integer_field:
        return None
    return (
        chl_file._dstring,
        ngates,
...
def _safe_chl_integer_scale(format_code, dat_factor, dat_bias, fld_factor):
    if not (np.isfinite(dat_factor) and np.isfinite(dat_bias) and np.isfinite(fld_factor)):
        return False
    if fld_factor == 0.0:
        return False

    info = np.iinfo(DATA_FORMAT[format_code])
    candidates = (1.0, float(info.max))
    with np.errstate(all="ignore"):
        scaled = [
            (raw_value * dat_factor + dat_bias) / fld_factor
            for raw_value in candidates
        ]
    return all(np.isfinite(value) for value in scaled)
