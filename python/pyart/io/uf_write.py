...
UF_RAY_MAP_RUST_MAX_RAYS = 1024 * 1024
...
def _uf_ray_num_to_sweep_num_rust(radar):
    if not isinstance(radar, Radar):
        return None
    if not isinstance(radar.nrays, (int, np.integer)):
        return None
    nrays = int(radar.nrays)
    if nrays < 0 or nrays > UF_RAY_MAP_RUST_MAX_RAYS:
        return None
    try:
        ...
        return None
    if (
        type(starts) is not np.ndarray
        or type(ends) is not np.ndarray
        or starts.dtype != np.int32
        or ends.dtype != np.int32
        or starts.ndim != 1
        or ends.ndim != 1
        or starts.shape != ends.shape
        or starts.size > UF_RAY_MAP_RUST_MAX_RAYS
        or not starts.flags.c_contiguous
        or not ends.flags.c_contiguous
    ):
        return None
    if starts.size and (
        starts.min() < 0
    ...
    kernel = _rust_kernel("_uf_ray_num_to_sweep_num_i32")
    if kernel is None:
        return None
    try:
        result = kernel(nrays, starts, ends)
    except Exception:
        return None
    if (
...
class UFRayCreator:
    ...
    def _calc_ray_num_to_sweep_num(radar):
        ...
        rust_result = _uf_ray_num_to_sweep_num_rust(radar)
