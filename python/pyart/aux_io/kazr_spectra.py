...
KAZR_RUST_MAX_OUTPUT_VALUES = 512 * 1024 * 1024
...
def _get_spectra(xrobj, time):
    ...
    rust_result = _get_spectra_rust(
        the_spectra_locs, xrobj.spectra.values, len(xrobj.speclength.values)
    )
...
def _get_spectra_rust(the_spectra_locs, spectra, npulses):
    args = _can_use_rust_get_spectra(the_spectra_locs, spectra, npulses)
    if args is None:
        return None
    kernel = _rust_kernel("_kazr_get_spectra")
    if kernel is None:
        return None
    return kernel(*args)


def _can_use_rust_get_spectra(the_spectra_locs, spectra, npulses):
    if not (
        type(the_spectra_locs) is np.ndarray
        and type(spectra) is np.ndarray
        and the_spectra_locs.flags.c_contiguous
        and spectra.flags.c_contiguous
    ):
        return None
    if the_spectra_locs.ndim != 1 or spectra.ndim != 2:
        return None
    if the_spectra_locs.dtype not in (np.float32, np.float64):
        return None
    ...
    try:
        ...
        return None
    if npulses < 0 or spectra.shape[1] != npulses:
        return None
    if the_spectra_locs.size * npulses > KAZR_RUST_MAX_OUTPUT_VALUES:
        return None

    indices = np.zeros(the_spectra_locs.shape, dtype=np.int64)
    missing = np.zeros(the_spectra_locs.shape, dtype=bool)
    nrows = spectra.shape[0]
    for i, locs in enumerate(the_spectra_locs):
        if locs == -9999.0:
            missing[i] = True
            continue
        if not np.isfinite(locs):
            return None
        try:
            index = int(locs)
        ...
        if index < 0:
            index += nrows
        indices[i] = index
    return spectra, indices, missing, npulses
