...
RAINBOW_RUST_MAX_OUTPUT_GATES = 512 * 1024 * 1024
...
def _get_data(rawdata, nrays, nbins, maxbin):
    ...
    rust_result = _get_data_rust(
        databin, datamin, datamax, datadepth, datatype, nrays, nbins, maxbin
    )
...
def _get_data_rust(databin, datamin, datamax, datadepth, datatype, nrays, nbins, maxbin):
    args = _can_use_rust_get_data(
        databin, datamin, datamax, datadepth, datatype, nrays, nbins, maxbin
    )
    if args is None:
        return None
    kernel_name, databin, nrays, nbins, maxbin, datamin, scale, fill_value, wrap_phidp = args
...
def _can_use_rust_get_data(databin, datamin, datamax, datadepth, datatype, nrays, nbins, maxbin):
    if not (type(databin) is np.ndarray and databin.flags.c_contiguous):
        return None
    if databin.ndim not in (1, 2):
        return None
    if databin.dtype == np.uint8:
        kernel_name = "_rainbow_wrl_get_data_u8"
    elif databin.dtype == np.uint16 and databin.dtype.isnative:
        kernel_name = "_rainbow_wrl_get_data_u16"
    else:
        return None

    dims = _rainbow_index_dims(nrays, nbins, maxbin)
    if dims is None:
        return None
    nrays, nbins, maxbin = dims
    if nbins > maxbin or databin.size != nrays * nbins:
        return None
    if nrays * maxbin > RAINBOW_RUST_MAX_OUTPUT_GATES:
        return None

    if type(datatype) is not str:
        return None
    wrap_phidp = datatype in {"PhiDP", "uPhiDP", "uPhiDPu"}
    fill_value = float(get_fillvalue())
    if not (
        np.isfinite(datamin)
        and np.isfinite(datamax)
        and np.isfinite(datadepth)
        and np.isfinite(fill_value)
    ):
        return None
    try:
        scale = (datamax - datamin) / 2**datadepth
    except OverflowError:
        return None
    if not np.isfinite(scale):
        return None
    if not _rainbow_scale_is_safe(databin, datamin, scale, fill_value):
        return None
    return kernel_name, databin, nrays, nbins, maxbin, datamin, scale, fill_value, wrap_phidp


def _rainbow_index_dims(nrays, nbins, maxbin):
    if any(isinstance(value, (bool, np.bool_)) for value in (nrays, nbins, maxbin)):
        return None
    try:
        dims = tuple(operator.index(value) for value in (nrays, nbins, maxbin))
    except TypeError:
        return None
...
def _rainbow_scale_is_safe(databin, datamin, scale, fill_value):
    if databin.size == 0:
        return True
    with np.errstate(all="ignore"):
        raw_min = float(databin.min())
        raw_max = float(databin.max())
        scaled = np.array(
            [datamin + raw_min * scale, datamin + raw_max * scale],
            dtype=np.float32,
        )
    return bool(np.all(np.isfinite(scaled)))
