...
IMAGE_MUTE_RUST_MAX_OUTPUT_VALUES = 512 * 1024 * 1024
...
def _image_mute_mask(data_to_mute, data_mute_by, mute_threshold, field_threshold):
    rust_result = _image_mute_mask_rust(
        data_to_mute, data_mute_by, mute_threshold, field_threshold
    )
    ...
    if field_filter is None:
        return mute_filter
    return mute_filter & field_filter


def _image_mute_mask_rust(data_to_mute, data_mute_by, mute_threshold, field_threshold):
    args = _can_use_rust_image_mute_mask(
        data_to_mute, data_mute_by, mute_threshold, field_threshold
    )
    if args is None:
        return None
    data_to_mute, data_mute_by, mute_threshold, has_field_threshold, field_threshold = args
    kernel = _rust_kernel("_image_mute_mask_dense_f64")
    if kernel is None:
        return None
    try:
        return kernel(
...
def _can_use_rust_image_mute_mask(
    data_to_mute, data_mute_by, mute_threshold, field_threshold
):
    if (
        type(data_to_mute) is not np.ndarray
        or type(data_mute_by) is not np.ndarray
        or data_to_mute.dtype != np.float64
        or data_mute_by.dtype != np.float64
        or data_to_mute.shape != data_mute_by.shape
        or not data_to_mute.flags.c_contiguous
        or not data_mute_by.flags.c_contiguous
    ):
        return None
    if data_to_mute.size > IMAGE_MUTE_RUST_MAX_OUTPUT_VALUES:
        return None

    mute_threshold = _coerce_image_mute_threshold(mute_threshold)
    if mute_threshold is None:
        return None
    if field_threshold is None:
        return data_to_mute, data_mute_by, mute_threshold, False, 0.0
    field_threshold = _coerce_image_mute_threshold(field_threshold)
    if field_threshold is None:
        return None
    return data_to_mute, data_mute_by, mute_threshold, True, field_threshold


def _coerce_image_mute_threshold(threshold):
    try:
        threshold_array = np.asarray(threshold)
    except (TypeError, ValueError):
        return None
    if threshold_array.ndim != 0 or threshold_array.dtype.kind in ("O", "S", "U"):
        return None
    try:
