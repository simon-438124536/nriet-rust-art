...
def cross_section_ppi(radar, target_azimuths, az_tol=None):
    ...
    for target_azimuth in sorted(target_azimuths):
        for sweep_slice in radar.iter_slice():
            ...
            nearest = _xsect_nearest_angle(sweep_azimuths, target_azimuth)
            if az_tol is None:
                if nearest is None:
                    d_az = np.abs(sweep_azimuths - target_azimuth)
                    ray_number = np.argmin(d_az)
                else:
                    ray_number, _ = nearest
...
def cross_section_rhi(radar, target_elevations, el_tol=None):
    ...
    for target_elevation in target_elevations:
        for sweep_slice in radar.iter_slice():
            ...
            nearest = _xsect_nearest_angle(sweep_elevations, target_elevation)
            if el_tol is None:
                if nearest is None:
...
def _xsect_nearest_angle(values, target):
    rust_result = _xsect_nearest_angle_rust(values, target)
...
def _xsect_nearest_angle_rust(values, target):
    if (
        type(values) is not np.ndarray
        or values.dtype != np.float64
        or values.ndim != 1
        or values.size == 0
        or values.size > XSECT_RUST_MAX_RAYS
        or not values.flags.c_contiguous
    ):
        return None
    try:
        target = _strict_float_scalar(target)
    except (TypeError, ValueError, OverflowError):
        return None

    kernel = _rust_kernel("_xsect_nearest_angle_f64")
    if kernel is None:
        return None
    try:
        index, distance = kernel(values, target)
    except Exception:
        return None
