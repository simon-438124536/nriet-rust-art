...
COLUMNSECT_RUST_MAX_RAYS = 1024 * 1024
...
def get_column_rays(radar, azimuth):
    ...
    rays = _get_column_rays_rhi_rust(radar, azimuth) if radar.scan_type == "rhi" else None
    if rays is None:
        rays = []
    ...
    if radar.scan_type == "rhi":
        ...
        for sweep in radar.iter_slice():
            ...
            nray = _get_column_ray_ppi_rust(sweep_azi, azimuth)
            if nray is None:
...
def get_sweep_rays(sweep_azi, azimuth, azimuth_spread=0):
    ...
    rust_result = _get_sweep_rays_rust(
        sweep_azi, azimuth, resolution * azimuth_spread
    )
...
def _get_column_ray_ppi_rust(sweep_azi, azimuth):
    if (
        type(sweep_azi) is not np.ndarray
        or sweep_azi.dtype != np.float64
        or sweep_azi.ndim != 1
        or sweep_azi.size == 0
        or sweep_azi.size > COLUMNSECT_RUST_MAX_RAYS
        or not sweep_azi.flags.c_contiguous
    ):
        return None
    try:
        azimuth = _strict_float_scalar(azimuth)
    except (TypeError, ValueError, OverflowError):
        return None
    if not np.isfinite(azimuth):
        return None

    kernel = _rust_kernel("_columnsect_nearest_ray_index_f64")
    if kernel is None:
        return None
    try:
        return int(kernel(sweep_azi, azimuth))
    except Exception:
        return None


def _get_column_rays_rhi_rust(radar, azimuth):
    azimuths = radar.azimuth["data"]
    if (
        type(azimuths) is not np.ndarray
        or azimuths.dtype != np.float64
        or azimuths.ndim != 1
        or azimuths.size > COLUMNSECT_RUST_MAX_RAYS
        or not azimuths.flags.c_contiguous
    ):
        return None
    try:
        azimuth = _strict_float_scalar(azimuth)
    ...
    try:
        nsweeps = radar.sweep_number["data"].shape[0]
    ...
    if (
        starts.ndim != 1
        or ends.ndim != 1
        or starts.size != nsweeps
        or ends.size != nsweeps
        or not np.issubdtype(starts.dtype, np.signedinteger)
        or not np.issubdtype(ends.dtype, np.signedinteger)
    ):
        return None
    starts = starts.astype(np.int64, copy=False)
    ends = ends.astype(np.int64, copy=False)

    kernel = _rust_kernel("_columnsect_get_column_rays_rhi_f64")
    if kernel is None:
        return None
    try:
        return kernel(azimuths, starts, ends, azimuth).tolist()
    except Exception:
        return None


def _get_sweep_rays_rust(sweep_azi, azimuth, spread_threshold):
    if (
        type(sweep_azi) is not np.ndarray
        or sweep_azi.dtype != np.float64
        or sweep_azi.ndim != 1
        ...
        return None
    if not np.isfinite(sweep_azi).all():
        return None
    try:
        azimuth = _strict_float_scalar(azimuth)
        spread_threshold = _strict_float_scalar(spread_threshold)
    except (TypeError, ValueError, OverflowError):
        return None
    if not (np.isfinite(azimuth) and np.isfinite(spread_threshold)):
        return None

    kernel = _rust_kernel("_columnsect_get_sweep_rays_f64")
    if kernel is None:
        return None
    try:
        centerline, spread = kernel(sweep_azi, azimuth, spread_threshold)
    except Exception:
        return None
    return centerline.tolist(), spread.tolist()


def _strict_float_scalar(value):
    if isinstance(value, (bool, np.bool_)) or type(value) is np.ndarray:
