...
def _rust_kernel():
    try:
        ...
        return None
    return getattr(rust, "estimate_noise_hs74", None)


def _can_use_rust(spectrum, navg, nnoise_min):
    return (
        isinstance(spectrum, np.ndarray)
        and spectrum.ndim == 1
        and spectrum.dtype == np.float64
        and spectrum.flags.c_contiguous
        and isinstance(nnoise_min, int)
        and nnoise_min >= 0
        and isinstance(navg, (int, float, np.integer, np.floating))
    )
...
def estimate_noise_hs74(spectrum, navg=1, nnoise_min=1):
    ...
    kernel = _rust_kernel()
    if kernel is not None and _can_use_rust(spectrum_array, navg, nnoise_min):
        return kernel(spectrum_array, float(navg), int(nnoise_min))
