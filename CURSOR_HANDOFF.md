...
- 当前真实工作目录：`F:\nriet-rust-art\repo`
- 当前工作分支：`rust-kernel-full-rewrite`
- 基线提交：`8f85750`（bootstrap）；P1 提交：`a4dfc4c`（`smooth_and_trim_scan` + 门禁/工具）
- 当前外部 oracle 源码包：`F:\nriet-rust-art\pyart-main.zip`（未解压时 API manifest diff 测试 skip）
...
2026-05-19 在 `rust-kernel-full-rewrite` 重跑结果：

```text
cargo fmt --check                     # pass
cargo test -q                         # 105 passed
source pytest tests -q                # 1635 passed, 980 skipped (test_native_extension x2 需 installed)
maturin build --release               # arm_pyart-0.1.0-cp312-cp312-win_amd64.whl
PYART_TEST_INSTALLED=1 pytest         # 2614 passed, 3 skipped
installed + RSTM_DATA_ROOT 闽侯数据    # 2617 passed
pytest tests/rstm -q                  # 9 passed, 4 skipped
```

`smooth_and_trim_scan` installed parity 使用 `atol=1e-14` 锁住 float64 累加 ULP（见 `test_phase_proc_smooth_scan.py`）。
...
- [x] 创建/确认工作分支 `rust-kernel-full-rewrite`
- [x] 提交 bootstrap 基线 `8f85750`
- [x] `cargo fmt --check` / `cargo test -q`
- [x] source pytest（见 §6；native_extension 需 wheel）
- [x] release wheel + 强制重装
- [x] installed pytest + 闽侯 RSTM
- [x] README Verification Results 段落

### P9 增量（2026-05-19）

- [x] `tools/api_manifest_compare.py`
- [x] `tests/api/test_api_manifest_oracle_diff.py`（oracle 未解压则 skip）
- [ ] CI 强制 manifest diff（需 `_oracle/`）

### P10 增量（2026-05-19）

- [x] `benchmarks/run_slice_benchmark.py`
- [ ] 全 slice benchmark + Spark 最终审查

### P1. `correct.phase_proc.smooth_and_trim_scan`（已完成 2026-05-19）

已实现：`_phase_proc_smooth_and_trim_scan_f64`、`tests/parity/test_phase_proc_smooth_scan.py`、Python dispatch。SciPy `convolve1d` mirror reflect 对齐；勿复用 1-D `smooth_and_trim_kernel`。
